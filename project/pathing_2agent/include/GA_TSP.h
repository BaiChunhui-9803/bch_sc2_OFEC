...
void GA_TSP::initialize_Algorithm(const std::vector<Node_TSP>& vec) {
    ...
    //标记起始点Begin1，Begin2，id为m_begin1_id，m_begin2_id
    for (int i = 0; i < UnitSize; i++) {
        if (m_city.at(i).m_type == Begin1) {
            m_begin1_id = m_city.at(i).m_mineral_id;
            std::swap(vec_pop.at(m_begin1_id), vec_pop.at(0));
        }
        if (m_city.at(i).m_type == Begin2) {
...
void GA_TSP::calculate_fitness() {
    ...
    for (i = 0; i < PopSize; i++) {
        m_distance[i] = 0;
        for (j = 1; j < UnitSize / 2; j++) {
            start = m_population[i][j - 1]; end = m_population[i][j];
            m_distance[i] += City_Distance[start][end];
        }
        for (j = UnitSize - 1; j > UnitSize / 2; j--) {
            start = m_population[i][j]; end = m_population[i][j - 1];
            m_distance[i] += City_Distance[start][end];
        }
...
void GA_TSP::output_draw() {
    std::ofstream fout;
    //第0代数据有误，剔除不进行输出
    fout.open("D:/bch_sc2_OFEC/sc2api/project/pathing_2agent/datafile/pop_best.txt");
    for (int i = 1; i < GenerationsMaxN; i++) {
        fout << i << ' ' << m_pop_best[i] << std::endl;
    }
    fout.close();

    fout.open("D:/bch_sc2_OFEC/sc2api/project/pathing_2agent/datafile/pop_avg.txt");
    for (int i = 1; i < GenerationsMaxN; i++) {
        fout << i << ' ' << m_pop_avg[i] << std::endl;
    }
    fout.close();

    fout.open("D:/bch_sc2_OFEC/sc2api/project/pathing_2agent/datafile/dist_gbest.txt");
    for (int i = 1; i < GenerationsMaxN; i++) {
...
void GA_TSP::save_best_to_node() {
    for (int i = 0; i < UnitSize; i++) {
        if (i == 0) {
            Node_TSP node(Begin1, 0, 0, City_Position[m_best_solution[i]].x, City_Position[m_best_solution[i]].y, m_city.at(0).m_z);
            ...
            Node_TSP node(Begin2, UnitSize - 1, UnitSize - 1, City_Position[m_best_solution[i]].x, City_Position[m_best_solution[i]].y, m_city.at(0).m_z);
            m_best_city.push_back(node);
    ...
    std::ofstream fout;
    fout.open("D:/bch_sc2_OFEC/sc2api/project/pathing_2agent/datafile/best_solution.txt");
