...
namespace OFEC {
	void MOLS_pop::initialize() {
		...
		int ind_id = 0;
		while (1) {
			for (; ind_id < m_inds.size(); ) {
				if (!check_order(*m_inds[ind_id]))
					std::cout << "order is not enough" << "\n";
				if (m_inds[ind_id]->rank() == rank_tmp) {
					m_archive.emplace_back(new individual_type(*m_inds[ind_id]));
				}
				ind_id++;
		...
		for (int i = 0; i < m_archive.size(); ++i) {
			m_inds.emplace_back(new individual_type(*m_archive[i]));
		}
		for (auto &i : m_inds) {
			i->evaluate();
		}
	...
	MOLS_pop::individual_type & MOLS_pop::select_sol(size_t obj_id)
	{
		/*Real max_obj = 0.0;
		int sol_id = 0;
		for (int i = 0; i < m_archive.size(); ++i) {
		...
		if (!check_order(*m_archive[sol_id]))
			std::cout << "order is not enough" << "\n";
		individual_type &sol_tmp = *m_archive[sol_id];
		return sol_tmp;
	}
	...
	void MOLS_pop::sort() {
		...
		std::vector<individual_type*> pop_;
		std::function<OFEC::dominationship(individual_type* const&, individual_type* const&)> comp = std::bind(&MOLS_pop::Pareto_compare, this, _1, _2);
		for (auto& i : m_inds)
			pop_.emplace_back(i.get());
		NS::fast_sort<individual_type*>(pop_, ranks_, comp);
		for (size_t i = 0; i < m_inds.size(); i++) {
			m_inds[i]->set_rank(ranks_[i]);
		}
	}
	...
	void MOLS_pop::construct_path_Online(individual_type & ind, size_t i)
	{
	...
	void MOLS_pop::updateSeqOnline(individual_type & ind, size_t i)
	{
		...
		m_best_inds.clear();
		m_best_inds.emplace_back(new individual_type(ind));
	...
	void MOLS_pop::updateSeqOnline_inserted(individual_type & ind, size_t i, std::pair<int, int> new_order)
	{
		DVRP_CAST->set_Online(false);
		DVRP_CAST->construct_path(ind);
		ind.evaluate();
	...
	void MOLS_pop::Online_ALS(individual_type & ind, size_t i)
	{
		...
		DCMOEA_ind<individual<DVRP::routes, Real>> best_ind_tmp = ind;
	...
	void MOLS_pop::ls_Rswap2_Online(individual_type & ind, size_t i)
	{
		auto i_t = ind;
		int selected_car = i;
		int begin_pos = -1;
	...
	void MOLS_pop::ls_maxDT_Online(individual_type & ind, size_t i)
	{
		auto i_t = ind;
		bool flag = true;
	...
	void MOLS_pop::ls_2opt_Online(individual_type & ind, size_t i)
	{
		auto i_t = ind;
		if (ind.variable().m_cus_order[i][0] != DVRP_CAST->get_depot()) {
	...
	EvalTag MOLS_pop::evolve() {
		...
		int eva = 0;
		/*while (eva <= max_eva) {*/
		individual_type &x_t = select_sol(0);
		for (int i = 0; i < DVRP_CAST->objective_size(); ++i) {
			//individual_type &x_t = select_sol(i);
			LS(x_t, i);
		}
		m_inds.clear();
		for (int i = 0; i < m_archive.size(); ++i) {
	...
	void MOLS_pop::LS(individual_type & offspring, size_t obj_id)
	{
		...
		if (nei_operator == 0) {
			...
			std::vector<individual_type> sol_tmp;
			...
			std::multimap<double, individual_type>sol_tmp_map;
			...
			if (!sol_tmp_map.empty()) {
				if (sol_tmp_map.begin()->first <= offspring_tmp.objective()[obj_id]) {
					offspring.solut() = sol_tmp_map.begin()->second;
				}
				else {
					updateA(sol_tmp_map.begin()->second);
					offspring.solut() = offspring_tmp;
				}
			}
			else
				offspring.solut() = offspring_tmp;
		...
		if (nei_operator == 1) {
			std::vector<individual_type> sol_tmp_t;
			std::vector<individual_type> sol_tmp;
			int selected_car = global::ms_global->m_uniform[caller::Algorithm]->next_non_standard<int>(0, offspring.variable().m_cus_order.size());
			while (offspring.variable().m_cus_order[selected_car].size() <= 3) {
				selected_car = global::ms_global->m_uniform[caller::Algorithm]->next_non_standard<int>(0, offspring.variable().m_cus_order.size());
	...
	void MOLS_pop::updateA(individual_type offspring)
	{
		if (m_archive.size() >= m_pop_size)
			return;
		m_archive.emplace_back(new individual_type(offspring));
	}

	bool MOLS_pop::feasible_check(individual_type & offspring)
	{
	...
	bool MOLS_pop::check_order(individual_type & offspring)
	{
		int num_order = 0;
	...
	dominationship MOLS_pop::e_Pareto_compare(individual_type* const&s1, individual_type* const&s2) {
	...
	dominationship MOLS_pop::Pareto_compare(individual_type * const & s1, individual_type * const & s2)
	{
		auto nor_obj_result = objective_compare<Real>(s1->objective(), s2->objective(), DVRP_CAST->opt_mode());
		auto vio_obj_result = objective_compare<Real>(s1->get_vio_obj(), s2->get_vio_obj(), optimization_mode::Minimization);
	...
	void MOLS::run_() {
		DCMOEA_ind<individual<DVRP::routes, Real>> best_ind = m_pop[0];
		DCMOEA_ind<individual<DVRP::routes, Real>> ini_best_ind = m_pop[0];
		m_pop.sort();
		...
		for (size_t i = 0; i < m_pop.size(); i++) {
			...
			pop_str << to_string(i) << "\t" << to_string(m_pop[i].rank()) << "\t" << t_route_length << "\t" << t_wait_time << "\t" << t_delay_time << "\t" << t_over_load << "\t" << t_over_time << "\t" << t_num_car << std::endl;
		}
		pop_ << pop_str.str();
		pop_.close();
		...
		while (!terminating()) {
			...
			for (size_t i = 0; i < m_pop.size(); i++) {
				...
				//std::cout << "pop[" + to_string(i) << "]\t" << to_string(m_pop[i].rank()) << "\t" << route_length << "\t" << wait_time << "\t" << delay_time << "\t" << over_load << "\t" << over_time << "\t" << num_car << std::endl;
				result_ << to_string(i) << "\t" << to_string(m_pop[i].rank()) << "\t" << route_length << "\t" << wait_time << "\t" << delay_time << "\t" << over_load << "\t" << over_time << "\t" << num_car << std::endl;
