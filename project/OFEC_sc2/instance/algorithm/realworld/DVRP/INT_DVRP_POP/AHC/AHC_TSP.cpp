...
namespace OFEC {
	void AHC_TSP::clustering(Clustering_result &result, int id_pro) {
		...
		cacul_dis_matrix(dis_temp, cluster_temp, m_data, id_pro);
		auto data = m_data;
		...
		while (cluster_temp.size() != 1) {
			...
			for (int i = 0; i < cluster_temp[mj].size(); ++i) {
				...
				if (is_valid(cluster_temp[mi], data, id_pro)) {
					flag_erase = true;
			...
			if (flag_erase == false) {
				...
				dis_temp.clear();
				cacul_dis_matrix(dis_temp, cluster_temp, data, id_pro);
			}
			else
			{
				m_cluster.push_back({});
		...
		for (int i = 0; i < m_cluster.size(); ++i) {
			...
			if (m_lkh == true)
				tsp_lkh(m_cluster[i], id_pro);
			else
				tsp_nearest(m_cluster[i], id_pro);
		}
		...
		for (size_t i = 0; i < m_cluster.size(); ++i) {
			...
			for (int j = 0; j < m_cluster[i].size(); ++j) {
				deliveryWeight = deliveryWeight + DVRP_CAST(id_pro).get_net().get_road_net()[m_cluster[i][j]]->datum_t.delivery_weight;
			}
			if (deliveryWeight > DVRP_CAST(id_pro).get_vehicle_property()[2].capacity * m_ca_cons_co) {
				is_over_load = true;
			}
	...
	void AHC_TSP::cacul_dis_matrix(std::vector<std::vector<Real>> &dis_temp, 
		const std::vector<std::vector<size_t>> &cluster_temp, const std::vector<size_t> &data, int id_pro) 
	{
		...
		for (int i = 0; i < cluster_temp.size(); ++i) {
			for (int j = 0; j < cluster_temp.size(); ++j) {
				...
				else if(j > i)
				{
					...
					if(m_lkh == true)
						d[i][j]=tsp_lkh(member, id_pro);
					else
						d[i][j] = tsp_nearest(member, id_pro);//计算cluster i和cluster j之间的距离
	...
	std::pair<Real, Real> AHC_TSP::tsp_nearest(std::vector<size_t>& member_index, int id_pro)
	{
		std::vector<size_t> tsp;
		std::vector<std::tuple<size_t, Real, Real>> member_cost;
		size_t start = DVRP_CAST(id_pro).get_depot();
		Real Time = DVRP_CAST(id_pro).get_depart_time();
		...
		while (!member_index.empty()) {
			...
			for (int i = 0; i < member_index.size(); ++i) {
				...
				get_cost(start, member_index[i], present_time, id_pro);//此处耗时可能太长
	...
	std::pair<Real, Real> AHC_TSP::tsp_lkh(std::vector<size_t>& member_index, int id_pro)
	{
		std::vector<size_t> tsp;
		//std::vector<std::tuple<size_t, Real, Real>> member_cost;
		size_t start = DVRP_CAST(id_pro).get_depot();
		...
		cal_tw_conflict(member_t,dis_matrix, id_pro);
		...
		for (int j = 0; j < member_t.size(); ++j) {
			for (int k = 0; k < member_t.size(); ++k) {
				...
				/*if (j == k) {
					...
					Totsp << DVRP_CAST(id_pro).get_cost_time(present_time, member_t[j], member_t[k]) << " ";
				}*/
			}
			Totsp << "\n";
		}
		...
		for (int i = 0; i < member_index.size()-1; ++i) {
			...
			get_cost(member_index[i], member_index[i + 1], present_time, id_pro);
			...
			diff_tw = 0.0; 
			beginTime = DVRP_CAST(id_pro).get_net().get_road_net()[member_index[i + 1]]->datum_t.begin_time;
			endTime = DVRP_CAST(id_pro).get_net().get_road_net()[member_index[i + 1]]->datum_t.end_time;
			if (beginTime >= present_time) {
				diff_tw = beginTime - present_time;
			}
			else if (beginTime < present_time && endTime >= present_time)
	...
	void AHC_TSP::cal_tw_conflict(const std::vector<size_t> &member_index, std::vector<std::vector<Real>> &dis_matrix, int id_pro)
	{
		//get conflict
		//member_index.insert(member_index.begin(),DVRP_CAST(id_pro).get_depot());
		dis_matrix.clear();
		...
		for (int i = 0; i < member_index.size(); ++i) {
			...
			for (int j = 0; j < member_index.size(); ++j) {
				...
				get_cost(member_index[i], member_index[j], present_time, id_pro);
				conflict_d[i][j] = present_time - Time;
				diff_tw = 0.0;
				
				beginTime = DVRP_CAST(id_pro).get_net().get_road_net()[member_index[j]]->datum_t.begin_time;
				endTime = DVRP_CAST(id_pro).get_net().get_road_net()[member_index[j]]->datum_t.end_time;
	...
	void AHC_TSP::get_cost(size_t s, size_t e, Real &present_time, int id_pro)
	{
	...
	const bool AHC_TSP::is_valid(const std::vector<size_t> &cluster, const std::vector<size_t> &data, int id_pro) const {
		/*bool is_over_load = false;
		double deliveryWeight = 0;
		for (size_t i = 0; i < cluster.size(); ++i) {

			deliveryWeight = deliveryWeight + DVRP_CAST(id_pro).get_net().get_road_net()[data[cluster[i]]]->datum_t.delivery_weight;
		...
		double over_load = 0;
		for (size_t i = 0; i < cluster.size(); ++i) {
			if (DVRP_CAST(id_pro).get_net().get_road_net()[data[cluster[i]]]->datum_t.is_served == false)
				deliveryWeight += DVRP_CAST(id_pro).get_net().get_road_net()[data[cluster[i]]]->datum_t.delivery_weight;
		}
		if (deliveryWeight > DVRP_CAST(id_pro).get_vehicle_property()[2].capacity * m_ca_cons_co) {
			over_load += deliveryWeight - DVRP_CAST(id_pro).get_vehicle_property()[2].capacity * m_ca_cons_co;
		}
		for (size_t i = 0; i < cluster.size(); ++i) {
			if (DVRP_CAST(id_pro).get_net().get_road_net()[data[cluster[i]]]->datum_t.is_served == false)
				deliveryWeight = deliveryWeight - DVRP_CAST(id_pro).get_net().get_road_net()[data[cluster[i]]]->datum_t.delivery_weight + DVRP_CAST(id_pro).get_net().get_road_net()[data[cluster[i]]]->datum_t.pick_weight;
			else
				deliveryWeight = deliveryWeight - 0 + DVRP_CAST(id_pro).get_net().get_road_net()[data[cluster[i]]]->datum_t.pick_weight;
