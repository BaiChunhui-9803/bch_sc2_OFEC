...
namespace OFEC {
	...
	void GA_DVRP::crossover(DCMOEA_ind<Individual<DVRP::routes>> & ind1, DCMOEA_ind<Individual<DVRP::routes>> & ind2, int id_rnd) {
		if (GET_RND(id_rnd).uniform.nextNonStd<Real>(0, 1) > m_cr) return;
		auto &c1 = ind1.variable().m_cus_order;
		//auto &c2 = ind2.variable().m_cus_order;
		int car_id_1 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, c1.size());
		int car_id_2 = car_id_1;
		while(car_id_2==car_id_1)
			car_id_2 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, c1.size());
		int pos1_1 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, c1[car_id_1].size() - 1);
		int pos1_2 = pos1_1;
		if (c1[car_id_1].size() > 3)
			while (pos1_1 == pos1_2)
				pos1_2 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, c1[car_id_1].size() - 1);

		int pos2_1 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, c1[car_id_2].size() - 1);
		int pos2_2 = pos2_1;
		if (c1[car_id_2].size() > 3)
			while (pos2_1 == pos2_2)
				pos2_2 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, c1[car_id_2].size() - 1);
	...
	void GA_DVRP::mutate(DCMOEA_ind<Individual<DVRP::routes>> &ind, int id_rnd) {
		if (GET_RND(id_rnd).uniform.nextNonStd<Real>(0, 1) > m_mr) return;
		auto &c1 = ind.variable().m_cus_order;
		int car_1 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, c1.size());
		int car_2 = car_1;
		while(car_2==car_1)
			car_2 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, c1.size());
		int pos1 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, c1[car_1].size() - 1);
		int pos2 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, c1[car_2].size() - 1);
