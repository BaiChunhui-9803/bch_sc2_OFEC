...
namespace OFEC {
	...
	//std::mutex mutex_AMP_out;
	template <typename Population1, typename Population2, typename Individual>
	AMP<Population1, Population2, Individual>::AMP(param_map & v) : algorithm(v.at("algorithm name")), \
		m_subpop((size_t)v.at("numSubPop"), (size_t)v.at("subpopulation size")), m_num_add((size_t)v.at("numSubPop") / 2), m_k((size_t)v.at("numSubPop")), \
	...
	template <typename Population1, typename Population2, typename Individual>
	EvalTag AMP<Population1, Population2, Individual>::evolve() {
		EvalTag tag = EvalTag::Normal;
	...
	void AMP<Population1, Population2, Individual>::output() {
		...
		path2 << g_working_dir << "/result/new/Alg4Epa/AMP/NET2_97/20w/curr_best_obj.txt";
