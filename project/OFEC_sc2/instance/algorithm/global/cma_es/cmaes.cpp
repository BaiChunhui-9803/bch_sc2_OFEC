 ...
 * */
{
  ...
  if (!t->sp.flgsupplemented) {
    ...
    if (!isNoneStr(t->sp.filename)) /* TODO: should this be done in readpara_SupplementDefaults? */
      cmaes_readpara_WriteToFile(&t->sp, ((std::string)OFEC::g_working_dir + "/instance/algorithm/ES/CMA_ES/actparcmaes.par").c_str());
  }
  ...
  if (t->sp.typicalXcase)
	  for (i = 0; i < N; ++i)
		  t->rgxmean[i] += t->sigma * t->rgD[i] * OFEC::global::ms_global->m_normal[OFEC::caller::Algorithm]->next();//cmaes_random_Gauss(&t->rand);
...
cmaes_SamplePopulation(cmaes_t *t)
{
  ...
  for (iNk = 0; iNk < t->sp.lambda; ++iNk)
    { /* generate scaled cmaes_random vector (D * z)    */
		for (i = 0; i < N; ++i)
			if (flgdiag)
				t->rgrgx[iNk][i] = xmean[i] + t->sigma * t->rgD[i] * OFEC::global::ms_global->m_normal[OFEC::caller::Algorithm]->next();// cmaes_random_Gauss(&t->rand);
			else
				t->rgdTmp[i] = t->rgD[i] * OFEC::global::ms_global->m_normal[OFEC::caller::Algorithm]->next();// cmaes_random_Gauss(&t->rand);
      if (!flgdiag)
...
void cmaes_ResizeLambda(cmaes_t* t, int lambda) {
    ...
    ////    t->sp.mu = lambda / 2;
    ////    cmaes_readpara_SetWeights(&t->sp, (&t->sp)->weigkey);
    ////    --t->rgFuncValue; t->rgFuncValue[0] = t->sp.lambda; ++t->rgFuncValue;
    ////    t->arFuncValueHist--;
    ////    t->arFuncValueHist[0] = (double)(10 + (int)ceil(3. * 10. * N / t->sp.lambda));
    ////    t->arFuncValueHist++;
    ////}
...
void cmaes_InitDistribution(cmaes_t* t, const double* rgFunVal)
{
    ...
    for (i = 0; i < t->sp.N; i++) {
        for (j = 0; j <= i; j++) {
            t->C[i][j] = 0;
            for (iNk = 0; iNk < t->sp.mu; iNk++) {
                t->C[i][j] += t->sp.weights[iNk] * (t->rgrgx[t->index[iNk]][i] - t->rgxmean[i]) * (t->rgrgx[t->index[iNk]][j] - t->rgxmean[j]);
            }
        }
    }
    t->flgIniphase = 0;
                          ...
                          char const *s3, char const *s4)
{
  ...
  */
  time_t t = time(NULL);
  FILE *fp = fopen(((std::string)OFEC::g_working_dir + "/instance/algorithm/ES/CMA_ES/errcmaes.err").c_str(), "a");
  if (!fp)
    {
      printf("\nFATAL ERROR: %s\n", s2 ? szCat(s1, s2, s3, s4) : s1);
