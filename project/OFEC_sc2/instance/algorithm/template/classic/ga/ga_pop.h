...
namespace OFEC {
	namespace GA {
		...
		public:
			...
			void add(const Individual& ind) override;
		...
		template<typename Individual>
		void population<Individual>::add(const Individual& ind) {
			OFEC::population<Individual>::add(ind);
			m_rand_seq.push_back(m_rand_seq.size());
		}

		template<typename Individual>
		void population<Individual>::remove(size_t idx) {
			OFEC::population<Individual>::remove(idx);
			m_rand_seq.resize(m_rand_seq.size() - 1);
			std::iota(m_rand_seq.begin(), m_rand_seq.end(), 0);
		}
	}
}
