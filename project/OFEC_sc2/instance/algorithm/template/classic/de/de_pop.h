...
namespace OFEC {
	enum class MutationDE { rand_1, best_1, target_to_best_1, best_2, rand_2, rand_to_best_1, target_to_rand_1, nrand_1 };
	template<typename IndType = IndDE>
	class PopDE : public Population<IndType> {
	protected:
		Real m_F, m_CR;
		MutationDE m_mutation_strategy;
		RecombineDE m_recombine_strategy;
	public:
		using population_type = OFEC::Population<IndType>;
		using population_type::m_inds;
		using population_type::m_best;
		using population_type::m_iter;
		using population_type::updateBest;
		...
		virtual void mutate(const int idx, int id_rnd, int id_pro);
		void mutate(int idx, const std::vector<int>& var, int id_rnd, int id_pro);
		void recombine(int idx, int id_rnd);
	...
	PopDE<IndType>::PopDE(size_t size_pop, int id_pro) :
		population_type(size_pop, id_pro, GET_CONOP(id_pro).numVariables()),
		m_F(0.5),
		m_CR(0.6),
	...
	template<typename IndType>
	PopDE<IndType>::PopDE(const PopDE& rhs) :
		population_type(rhs),
		m_F(rhs.m_F),
		m_CR(rhs.m_CR),
	...
	template<typename IndType>
	PopDE<IndType>::PopDE(PopDE&& rhs) noexcept :
		population_type(std::move(rhs)),
		m_F(rhs.m_F),
		m_CR(rhs.m_CR),
		m_mutation_strategy(rhs.m_mutation_strategy),
		m_recombine_strategy(rhs.m_recombine_strategy) {}
	...
	template<typename IndType>
	PopDE<IndType>& PopDE<IndType>::operator=(PopDE&& rhs) noexcept {
		if (this == &rhs) return *this;
	...
	template<typename IndType>
	void PopDE<IndType>::mutate(const int idx, int id_rnd, int id_pro) {
		...
		case MutationDE::best_1:
			...
			break;
		case MutationDE::target_to_best_1:
			select(idx, 2, ridx, id_rnd);
		...
		case MutationDE::rand_to_best_1:
			...
			break;
		case MutationDE::target_to_rand_1:
			select(idx, 3, ridx, id_rnd);
			m_inds[idx]->mutate(m_F, m_inds[idx].get(), m_inds[ridx[0]].get(), m_inds[idx].get(), id_pro, m_inds[ridx[1]].get(), m_inds[ridx[2]].get());
			break;
		case MutationDE::nrand_1:
			int nearest = this->nearestNeighbour(idx, id_pro, 1).begin()->second;
			select(idx, 2, ridx, id_rnd);
			m_inds[idx]->mutate(m_F, m_inds[nearest].get(), m_inds[ridx[0]].get(), m_inds[ridx[1]].get(), id_pro);
			break;
		}
	}

	template<typename IndType>
	void PopDE<IndType>::recombine(int idx, int id_rnd) {
		m_inds[idx]->recombine(m_CR, m_recombine_strategy, id_rnd);
	}

	template<typename IndType>
	void PopDE<IndType>::mutate(int idx, const std::vector<int>& var, int id_rnd, int id_pro) {
		std::vector<size_t> ridx;
		switch (m_mutation_strategy) {
		case MutationDE::rand_1:
			select(idx, 3, ridx, id_rnd);
	...
	template<typename IndType>
	EvalTag PopDE<IndType>::evolve(int id_pro, int id_alg, int id_rnd) {
		if (m_inds.size() < 5) {
			throw MyExcept("the population size cannot be smaller than 5@DE::PopDE<IndType>::evolve()");
		}
		EvalTag tag = EvalTag::Normal;
		if (m_mutation_strategy == MutationDE::best_1 ||
			m_mutation_strategy == MutationDE::target_to_best_1 ||
	...
	template<typename IndType>
	void PopDE<IndType>::select(int base_, int number, std::vector<size_t>& result, int id_rnd) {
	...
	template<typename IndType>
	void PopDE<IndType>::selectInNeighborhood(int number, std::vector<size_t> candidate, std::vector<size_t>& result, int id_rnd) {
		if (result.size() != number)	result.resize(number);
		for (size_t i = 0; i < number; ++i) {
