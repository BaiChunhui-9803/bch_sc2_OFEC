...
namespace OFEC {
	...
	modified_shekel::modified_shekel(const ParamMap &v) : 
		modified_shekel((v.at("problem name")), (v.at("number of variables")), 1) {
	...
	void modified_shekel::initialize() {
		if (m_num_vars > 5) throw myexcept("number of dim must be <=5@ modified_shekel::modified_shekel");
		setDomain(0.0, 11.0);
		...
		m_variable_accuracy = 1.e-3;
		 ...
		 //1 gopt+7 lopt
		setOriginalGlobalOpt(m_a[6]);
		for (size_t m = 0; m < 8; ++m) {
			if (m == 6) continue;
			setOriginalGlobalOpt(m_a[m]);
		}
		m_optima = m_original_optima;
		m_initialized = true;
	}
	EvalTag modified_shekel::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real s = 0;
