#include "F12_composition2015_C4.h"
...
namespace OFEC {
	namespace CEC2015 {
		F12_composition2015_C4::F12_composition2015_C4(const ParamMap &v) :
			F12_composition2015_C4((v.at("problem name")), (v.at("number of variables")), 1) {
			
		}
		F12_composition2015_C4::F12_composition2015_C4(const std::string &name, size_t size_var, size_t size_obj) :problem(name, size_var, size_obj), \
			composition_2015(name, size_var, size_obj) {
			
		}

		void F12_composition2015_C4::setFunction() {
			basic_func f(5);
		...
		void F12_composition2015_C4::initialize() {
		...
		EvalTag F12_composition2015_C4::evaluateObjective(Real *x, std::vector<Real> &obj) {
		...
		bool F12_composition2015_C4::load_optima(const std::string &path) {
			std::string s;
			std::stringstream ss;
		...
		void F12_composition2015_C4::load_optima_(const std::string &path) {
			std::ifstream in;
			in.open(path);
			if (in.fail()) {
				set_optima();
		...
		void F12_composition2015_C4::set_optima() {
			//  to do ..
			throw myexcept("Waiting to do !");
		}

		bool F12_composition2015_C4::loadTranslation(const std::string &path) {
			std::string s;
			std::stringstream ss;
			ss << ".txt";
			s = ss.str();
		...
		void F12_composition2015_C4::setTranslation() {
		...
		void F12_composition2015_C4::evaluate_optima() {
            Solution<VarVec<Real>, Real> s(m_num_objs, num_constraints(), m_num_vars);
            for (size_t i = 0; i < m_optima.number_variable(); ++i) {
		...
		void F12_composition2015_C4::rotate(size_t num, Real *x) {
		...
		void F12_composition2015_C4::scale(size_t num, Real *x) {
			for (size_t i = 0; i<m_num_vars; ++i)
				x[i] /= m_function[num]->scale();
		}
		void F12_composition2015_C4::set_weight(std::vector<Real>& weight, const std::vector<Real>&x) {
