...
namespace OFEC {
		...
		,  int peakNumChangeMode,  bool flagNoise,  bool flagTimelinkage) :dynamic_continuous(name, rDimNumber, rNumPeaks, numObj)
	{
		...
		for (auto i = 0; i < m_peak.size(); i++) {
			m_rotM[i].resize(m_num_vars, m_num_vars);
		}
		setDomain(-5, 5);
		m_max_height = 100;
		m_min_height = 10;
		...
		std::vector<std::vector<Real>> position(m_peak.size(),std::vector<Real>(m_num_vars));
	...
	void real_DBG::change_location(Real angle) {
		...
		std::vector<Real> gene (m_num_vars);
		for (int i = 0; i < m_peak.size(); i++) {
			...
			m.set_row(m_peak[i].data(), m_num_vars);
			m *= m_rotM[i];
			gene = m[0].vect();
			repair(gene.data());
			m_peak[i].vect() = gene;
