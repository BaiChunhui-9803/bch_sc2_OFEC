...
namespace OFEC {
	...
	composition_DBG::composition_DBG(const std::string &name, int rDimNumber, int rNumPeaks, int frequency, ChangeType rT, ComDBGFuncID rF, \
		...
		matrix m( 1, m_num_vars);
		Real *gene = new Real[m_num_vars];
		for (int i = 0; i < m_peak.size(); i++) {
			for (int j = 0; j < m_num_vars; j++) { // calculate the estimate max value of funciton i
				gene[j] = m_domain[j].limit.second;
				gene[j] /= m_stretch_severity[i];
			}
			m.set_row(gene, m_num_vars);
			m *= m_rotM[i];
	...
	void composition_DBG::set_rotation_matrix() {
		...
		matrix I(m_num_vars, m_num_vars);

		std::vector<int>  d(m_num_vars);
		global::ms_global->m_uniform[caller::Problem]->shuffle(d.begin(), d.end());
		for (int i = 0; i < m_peak.size(); i++) {
			for (int j = 0; j + 1 < m_num_vars; j += 2) {
	...
	EvalTag composition_DBG::evaluateObjective(Real* x_, std::vector<Real>& obj_) {
		...
		for (int i = 0; i < m_peak.size(); i++) { // calculate weight for each function		
			...
			if (width[i] != 0)	width[i] = exp(-std::sqrt(width[i] / (2 * m_num_vars*m_converge_severity[i] * m_converge_severity[i])));
		}

		for (int i = 0; i < m_peak.size(); i++) { // calculate objective value for each function
			for (int j = 0; j < m_num_vars; j++)	// calculate the objective value of tranformation function i
				x[j] = (x[j] - m_peak[i][j]) / m_stretch_severity[i];//((1+fabs(m_peak[i][j]/mp_searchRange[j].limit.second))*
			matrix m(1, m_num_vars);
			m.set_row(x, m_num_vars);
			m *= m_rotM[i];
	...
	Real composition_DBG::fAckley(Real *x) {
		...
		fitness = -20 * exp(-0.2*sqrt(s1 / m_num_vars)) - exp(s2 / m_num_vars) + 20 + OFEC_E;
		return fitness;
	}
	...
	void  composition_DBG::change_num_peak() {
		composition_DBG r_dbg("compositionDBG", m_num_vars, m_temp_num_peak, m_frequency,m_change.type, ms_fun_idx,
