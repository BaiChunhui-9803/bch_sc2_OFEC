...
namespace OFEC {
	...
	HE7::HE7(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {
		...
		for (size_t m = 0; m < m_num_objs; ++m)
		{
			if (m < 1) {
				...
				for (size_t i=2; i < m_num_vars; i += 2) {
					obj[m] += std::pow(x[i] - (0.3*std::pow(x[0],2)*std::cos(24*OFEC_PI*x[0]+4*(i+1)*OFEC_PI/m_num_vars)\
						+0.6*x[0])* std::cos(6 * OFEC_PI*x[0] + (i + 1)*OFEC_PI / m_num_vars), 2);
				}
				...
				for (size_t i=1; i < m_num_vars; i += 2) {
					gt += std::pow(x[i] - (0.3*std::pow(x[0], 2)*std::cos(24 * OFEC_PI*x[0] + 4 * (i + 1)*OFEC_PI / m_num_vars)\
						+ 0.6*x[0])* std::sin(6 * OFEC_PI*x[0] + (i + 1)*OFEC_PI / m_num_vars), 2);
				}
