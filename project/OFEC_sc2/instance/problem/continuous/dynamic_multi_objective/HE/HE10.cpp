...
namespace OFEC {
	...
	HE10::HE10(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {

	}

	void HE10::initialize() {
		...
		for (size_t m = 0; m < m_num_objs; ++m)
		{
			if (m < 1) {
				...
				for (size_t i=1; i < m_num_vars; i += 2) {
					gt +=pow(x[i] - std::sin(6 * OFEC_PI*x[0] + (i + 1)*OFEC_PI / m_num_vars), 2);
				}
				gt = 2 - pow(x[0], 2) + 2. / floor(m_num_vars / 2.)*gt;
