...
namespace OFEC {
	...
	T2::T2(const std::string & name, size_t size_var,size_t size_obj) : problem(name, size_var, size_obj), DMOPs(name, size_var,size_obj) {

	}

	void T2::initialize() {//all variables are in（0,1）,need revise
	...
	EvalTag T2::evaluateObjective(Real *x, std::vector<Real> &obj) {
		...
		Real m_t = floor(m_num_objs*fabs(sin(0.5*OFEC_PI*t)));
		Real d2_t = floor(m_num_vars*fabs(pow(cos(2 * t), 3)));
		Real g = 0;
