...
namespace OFEC::DMOP {
	F6::F6(param_map & v) : F6(v.at("problem name"), v.at("number of variables")) {

	}
	F6::F6(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {
	...
	//f1=x1, f2=g*h
	EvalTag F6::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real t = get_time();
		...
		Real a = 2 + 2 * std::cos(1.5*OFEC_PI*t)*std::sin(0.5*OFEC_PI*t);
		Real b = 2 + 2 * std::cos(1.5*OFEC_PI*t)*std::cos(0.5*OFEC_PI*t);
