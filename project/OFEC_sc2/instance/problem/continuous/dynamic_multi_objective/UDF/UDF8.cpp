...
namespace OFEC {
	UDF8::UDF8(param_map & v) : UDF8(v.at("problem name"), v.at("number of variables")) {

	}
	UDF8::UDF8(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {

	}

	void UDF8::initialize() {
	...
	EvalTag UDF8::evaluateObjective(Real *x, std::vector<Real> &obj) {//recommend the number of variables is 20
		Real t = get_time();
		...
		for (size_t m = 0; m < m_num_objs; ++m)
		{
			if (m < 1) {
				...
				for (size_t i=2; i < m_num_vars; i += 2) {
					obj[m] += std::pow(x[i] - std::sin(6 * OFEC_PI*x[0] + \
						(i + 1 + std::ceil(m_num_vars*std::sin(0.5*OFEC_PI*m_count[0]))) * OFEC_PI / m_num_vars)- std::sin(0.5*OFEC_PI*m_count[2]), 2);
				}
				...
				for (size_t i=1; i < m_num_vars; i += 2) {
					obj[m] += std::pow(x[i] - std::sin(6 * OFEC_PI*x[0] + \
						(i + 1 + std::ceil(m_num_vars*std::sin(0.5*OFEC_PI*m_count[0]))) * OFEC_PI / m_num_vars)- std::sin(0.5*OFEC_PI*m_count[1]), 2);
				}
