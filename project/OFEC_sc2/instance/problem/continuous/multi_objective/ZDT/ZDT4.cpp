#include "ZDT4.h"

namespace OFEC {
	ZDT4::ZDT4(param_map & v) :  ZDT4(v.at("problem name"), v.at("number of variables")) { //param_numDim = 10 is suggested
		
	}
	ZDT4::ZDT4(const std::string & name, size_t size_var) : problem(name, size_var, 2), ZDT(name, size_var) {
		for (int count = 1; count < size_var; ++count)
			m_domain.setDomain(-5., 5., count);
		
	}

	EvalTag ZDT4::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real g = 0;
		for (size_t n = 1; n<m_num_vars; n++)
			g = g + (pow(x[n], 2) - 10 * cos(4 * OFEC_PI*x[n]));
		g = 1 + 10 * (m_num_vars - 1) + g;
		obj[0] = x[0];
		obj[1] = g*(1 - std::sqrt(x[0] / g));
		return EvalTag::Normal;
	}
}
