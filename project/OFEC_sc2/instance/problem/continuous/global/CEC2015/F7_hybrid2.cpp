...
namespace OFEC {
	namespace CEC2015 {
		F7_hybrid2::F7_hybrid2(const ParamMap &v) :
			F7_hybrid2((v.at("problem name")), (v.at("number of variables")), 1) {
			
		}
		F7_hybrid2::F7_hybrid2(const std::string &name, size_t size_var, size_t size_obj) :problem(name, size_var, size_obj), \
			hybrid(name, size_var, size_obj) {
			
		}
		void F7_hybrid2::setFunction() {
			size_t i, tmp;
		...
		EvalTag F7_hybrid2::evaluateObjective(Real *x, std::vector<Real> &obj) {
