...
namespace OFEC {
	namespace CEC2005 {
		void HybridCompositionNoisy::initialize_() {
		...
		void HybridCompositionNoisy::evaluateObjective(Real *x, std::vector<Real> &obj) {
			Composition::evaluateObjective(x, obj);
			obj[0] = obj[0] * noise() + 120.;
		}

		Real HybridCompositionNoisy::noise() {
			return (1 + 0.2 * fabs(GET_RND(m_id_rnd).normal.next()));
		}

		void HybridCompositionNoisy::setFunction() {
			basic_func f(5);
