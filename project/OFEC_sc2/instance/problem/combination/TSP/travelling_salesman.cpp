...
namespace OFEC {
	void TravellingSalesman::initialize_() {
		...
		//if (m_num_cities <= 20) global::ms_sample_fre = 10;
		//else if (m_num_cities <= 50) global::ms_sample_fre = 100;
		//else if (m_num_cities <= 100) global::ms_sample_fre = 500;
		//else if (m_num_cities <= 200) global::ms_sample_fre = 10000;
		//else if (m_num_cities <= 300) global::ms_sample_fre = 20000;
		//else if (m_num_cities <= 400) global::ms_sample_fre = 50000;
		//else if (m_num_cities <= 500) global::ms_sample_fre = 100000;
		//else if (m_num_cities <= 600) global::ms_sample_fre = 200000;
		//else if (m_num_cities <= 700) global::ms_sample_fre = 500000;
		//else if (m_num_cities <= 800) global::ms_sample_fre = 1000000;
		//else if (m_num_cities <= 900) global::ms_sample_fre = 2000000;
		//else if (m_num_cities <= 1000) global::ms_sample_fre = 3000000;
		//else if (m_num_cities <= 2000) global::ms_sample_fre = 5000000;
	}
	...
	Real TravellingSalesman::variableDistance(const VarBase& s1, const VarBase& s2) const {
		static std::vector<std::vector<bool>> mvv_s1_edge(m_num_cities); // a temporary matrix to store the edges of s1, thus helping calculate the distance between two solutions
...
#define	strtok_s strtok_r
		...
		while (std::getline(infile, Line)) {
			...
			if (!strcmp(Keyword, "NAME"))
				...
				while (infile >> Line)
				{
					...
					++i;
					if (i == (size_t)m_num_cities) break;
				}
				break;
			}
	...
	bool TravellingSalesman::isOptimaGiven() const {
	...
	size_t TravellingSalesman::numOptimaFound(const std::list<std::unique_ptr<SolBase>> &candidates) const {
		if (m_optima.isObjectiveGiven() && !candidates.empty() && candidates.front()->objectiveDistance(m_optima.objective(0)) < m_objective_accuracy)
			return 1;
		else
			return 0;
	}
