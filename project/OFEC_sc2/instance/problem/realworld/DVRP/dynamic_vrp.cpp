...
namespace OFEC {
	namespace DVRP {
		...
		//dynamic_vrp::dynamic_vrp(const ParamMap &v) : dynamic_vrp(v.at("problem name"), v.at("numCus"), v.at("numNewCus"), v.at("numObj"), v.at("dataFile1"), v.at("dataFile2"),v.at("dataFile3")) {
		...
		void dynamic_vrp::initialize_() {
			...
			m_road_net.select_customers(m_depot, m_current_order_num, m_future_order_num, m_depart_time, m_dynamic_order_endtime, m_id_rnd);//(depot, static_order, dynamic_order, depart_time, dynamic_endtime)
		...
		std::vector<bool> dynamic_vrp::is_valid(SolBase &s) const {
		...
		void dynamic_vrp::initSolution(SolBase &s, int id_rnd) const{
			routes &x = dynamic_cast<Solution<routes> &>(s).variable();
			auto cus_order_temp = m_current_cus_order;
			GET_RND(id_rnd).uniform.shuffle(cus_order_temp.begin(), cus_order_temp.end());
		...
		void dynamic_vrp::construct_path(SolBase & s) {
			routes &x = dynamic_cast<Solution<routes> &>(s).variable();
			x.m_path.clear();
		...
		void dynamic_vrp::construct_path(SolBase & s) const{
			routes &x = dynamic_cast<Solution<routes> &>(s).variable();
			x.m_path.clear();
			x.m_path_node_type.clear();
			x.m_path.resize(x.m_cus_order.size());
		...
		bool dynamic_vrp::same(const SolBase &s1, const SolBase &s2) const {
		...
		void dynamic_vrp::evaluate_(SolBase &s, bool effective) {
			routes &x = dynamic_cast<Solution<routes> &>(s).variable();
		...
		Real dynamic_vrp::variableDistance(const SolBase &s1, const SolBase &s2) const {
		...
		Real dynamic_vrp::variableDistance(const VarBase &s1, const VarBase &s2) const {
			std::vector<std::vector<bool>> s1_edge(m_nodes_size);
			...
			for (size_t ii = 0; ii < m_road_net.get_road_net().size(); ++ii) {
				for (size_t jj = 0; jj < m_road_net.get_road_net().size(); ++jj) {
					if (ii == jj) {
						...
						//all_path.push_back({ INT_MAX });
						//shortest_path_t << INT_MAX << "\n";
						//shortest_path_str << INT_MAX << "\n";
					}
					else
					{
						Real sum = 0.0;
