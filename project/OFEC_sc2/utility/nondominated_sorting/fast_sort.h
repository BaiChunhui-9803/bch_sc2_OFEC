...
namespace OFEC {
	namespace NS {
		...
		template<typename ObjectiveType>
		void fast_sort_p_compare(const std::vector<std::vector<ObjectiveType>*>& data, const std::vector<OptMode>& opt_mode, int popsize, const std::vector<int>&& ks, std::vector<int>& rank_, std::vector<int>& count, std::vector<std::vector<int>>& cset) {
			for (int k : ks) {
				for (int j = 0; j < popsize; j++) {
					if (k != j) {
						auto compare_result = objectiveCompare<ObjectiveType>(*data[j], *data[k], opt_mode);
