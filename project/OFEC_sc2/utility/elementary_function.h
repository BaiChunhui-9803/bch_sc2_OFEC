...
namespace OFEC {
	...
	public:
		...
		elementary_function(const ParamMap& v) :
			m_from_x_range(std::get<Real>(v.at("from_x_down")), std::get<Real>(v.at("from_x_up"))),
			m_to_x_range(std::get<Real>(v.at("to_x_down")), std::get<Real>(v.at("to_x_up"))),
			m_to_y_range(std::get<Real>(v.at("to_y_down")), std::get<Real>(v.at("to_y_up")))
		{}
