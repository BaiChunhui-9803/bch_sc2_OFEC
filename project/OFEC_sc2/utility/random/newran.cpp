...
namespace OFEC {
	...
	double RandBase::density(double) const {
		...
		throw MyExcept("density function not defined@RandBase::density");
	...
	{
		if (!m_seed)
			throw MyExcept("RandBase number generator not initialised@RandBase::next");
										  ...
										  // s must be between 0 and 1
	{
		...
		if (s >= 1.0 || s <= 0.0)
			throw MyExcept("Newran: seed out of range@RandBase::RandBase");
