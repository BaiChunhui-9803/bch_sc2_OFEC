...
namespace OFEC {
	namespace NS {
		namespace FilterSort {
			template<typename T>
			bool greater(const std::vector<T>* a, const std::vector<T>* b, const int obj_idx, const std::vector<OptMode>& opt_mode) {
				size_t obj_num = a->size();
				for (size_t i = 0; i < obj_num; ++i) {
					int idx = (i + obj_idx) % obj_num;
					if (opt_mode[idx] == OptMode::Minimize) {
			...
			template<typename T>
			void merge(const std::vector<std::vector<T>*>& data, const int obj_idx, std::vector<int>& A, std::vector<int>& B, int l, int m, int r, const std::vector<OptMode>& opt_mode) {
				int i = l, j = m;
				for (int k = l; k < r; ++k) {
			...
			template<typename T>
			void mergeSort(const std::vector<std::vector<T>*>& data, const int obj_idx, std::vector<int>& B, std::vector<int>& A, int l, int r, const std::vector<OptMode>& opt_mode)
			{
				if (r - l < 2)
					return;
				int m = (r + l) / 2;
				mergeSort<T>(data, obj_idx, A, B, l, m, opt_mode);
				mergeSort<T>(data, obj_idx, A, B, m, r, opt_mode);

				merge<T>(data, obj_idx, B, A, l, m, r, opt_mode);
			}

			template<typename T>
			void merge_sort(const std::vector<std::vector<T>*>& data, std::vector<int>& index, const int obj_idx, const std::vector<OptMode>& opt_mode) {
				...
				std::vector<int> B(A);
				mergeSort<T>(data, obj_idx, B, A, 0, N, opt_mode);
				for (int i = 0; i < N; ++i)
					index[i] = A[i];
			}

			template<typename T>
			void parallel_mergesort(const std::vector<std::vector<T>*>& data, std::vector<std::vector<int>>& SeqByObj, std::vector<int> ObjIdxs, const std::vector<OptMode>& opt_mode) {
				for (int ObjIdx : ObjIdxs)
					merge_sort<T>(data, SeqByObj[ObjIdx], ObjIdx, opt_mode);
			}
			...
			inline void parallel_filter(std::vector<int> candidates, const std::vector<List>& SeqByObj_Lists, const std::vector<int>& MaxIdxs, const std::vector<int>& MinIdxs, const int m, const std::vector<std::vector<int>>& SolStas, bool* InCurFront) {
		...
		template<typename T>
		int filter_sort(const std::vector<std::vector<T>*>& data, std::vector<int>& rank, const std::vector<OptMode>& opt_mode) {
			...
			for (size_t j = 0; j < M; j++) {
				FilterSort::merge_sort<T>(data, SeqByObj[j], j, opt_mode);
			}
			...
			while (num_not_ranked > 0) {
				...
				for (int candidate : CurCandidate) {
					bool FlagInCurRank(true); // whether candidate is in current rank 
					for (const FilterSort::Node* iter = SeqByObj_Lists[MinIdxs[candidate]].begin(); iter != nullptr; iter = iter->m_next) {
						if (iter->m_value == candidate)
							break;
						else {
							if (SolStas[iter->m_value][MaxIdxs[iter->m_value]] > SolStas[candidate][MaxIdxs[iter->m_value]])
		...
		template<typename T>
		int filter_sort_p(const std::vector<std::vector<T>*>& data, std::vector<int>& rank, const std::vector<OptMode>& opt_mode, size_t num_task = -1) {
			if (num_task == -1)
			...
			for (int i = 0; i < num_task; ++i) {
				...
				thrd.push_back(std::thread(FilterSort::parallel_mergesort<T>, std::cref(data), std::ref(SeqByObj), ObjIdxs, std::cref(opt_mode)));
			}
			for (auto& t : thrd)
				t.join();
			...
			while (num_not_ranked > 0) {
				...
				for (int i = 0; i < num_task; ++i) {
					...
					thrd.push_back(std::thread(FilterSort::parallel_filter, candidates, std::cref(SeqByObj_Lists), std::cref(MaxIdxs), std::cref(MinIdxs), M, std::cref(SolStas), InCurFront));
				}
				for (auto& t : thrd)
					t.join();
				// set rank for current Front
