...
namespace OFEC {
	...
	template<typename Individual>
	void APD<Individual>::updateData(const population<Individual> &pop) {
		m_D.clear();
		std::set<std::vector<real>> D_x;    // for uniqueness
		for (size_t i = 0; i < pop.size(); i++)	{
			if (D_x.count(pop[i].variable().vect()) == 0) {
				m_D.push_back(&pop[i].solut());
	...
	template<typename Individual>
	void APD<Individual>::updateData(const std::vector<Individual> &inds) {
		m_D.clear();
		std::set<std::vector<real>> D_x;    // for uniqueness
		for (size_t i = 0; i < inds.size(); i++) {
			if (D_x.count(inds[i].variable().vect()) == 0) {
				m_D.push_back(&inds[i].solut());
	...
	template<typename Individual>
	void APD<Individual>::clustering() {
	...
	template<typename Individual>
	void APD<Individual>::updateDistance(const std::unordered_set<size_t>& D_c) {
	...
	template<typename Individual>
	void APD<Individual>::landscapeCutting(std::unordered_set<size_t> &D_c, real eta) {
	...
	template<typename Individual>
	void APD<Individual>::peakDetection(std::unordered_set<size_t> D_c) {
	...
	template<typename Individual>
	void APD<Individual>::normManhDisMat(const std::unordered_set<size_t>& D_c) {
	...
	template<typename Individual>
	real APD<Individual>::minManhDis(const std::unordered_set<size_t>& D_c, size_t i) {
		auto iter = D_c.begin();
		if (*iter == i) iter++;
