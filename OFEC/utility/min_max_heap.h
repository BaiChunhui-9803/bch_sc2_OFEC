...
namespace OFEC {
	...
	template<typename T>
	MinMaxHeap<T>::MinMaxHeap(const std::vector<T> &data) : m_data(data) {
		build();
	} 

	template<typename T>
	void MinMaxHeap<T>::updateValue(size_t id, const T &val) {
		T pre_val = m_data[id];
		m_data[id] = val;
		if (pre_val > val)
			checkUp(id);
	...
	template<typename T>
	void MinMaxHeap<T>::remove(size_t id) {
		if (m_pos[id] == -1)
			return;
	...
	template<typename T>
	void MinMaxHeap<T>::add(const T &val) {
	...
	template<typename T>
	size_t MinMaxHeap<T>::top() {
		return m_seq.front();
	}
	
	template<typename T>
	inline void MinMaxHeap<T>::checkUp(size_t id) {
		int child = m_pos[id];
	...
	template<typename T>
	inline void MinMaxHeap<T>::checkDown(size_t id) {
		int parent = m_pos[id];
