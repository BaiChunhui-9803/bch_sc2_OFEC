...
namespace OFEC {
	namespace CEC2015 {
		F14_composition2015_C6::F14_composition2015_C6(const ParamMap &v) :
		...
		void F14_composition2015_C6::setFunction() {
			...
			for (size_t i = 0; i < m_num_function; ++i) {
				if(i/2 == 0) m_function[i] = dynamic_cast<function*>(f[0]("", m_num_vars, m_num_objs));
				else  m_function[i] = dynamic_cast<function*>(f[1]("", m_num_vars, m_num_objs));
		...
		void F14_composition2015_C6::initialize() {  // don't set optima
			...
			if (m_num_vars == 10) m_pre_opt_distance = 113;
			else if(m_num_vars == 20) m_pre_opt_distance = 183;
			else if(m_num_vars == 30) m_pre_opt_distance = 285;
			
			add_tag(problem_tag::MMOP);
			m_initialized = true;
		}
		EvalTag F14_composition2015_C6::evaluateObjective(Real *x, std::vector<Real> &obj) {
