...
namespace OFEC::DMOP {
	F5::F5(param_map & v) : F5(v.at("problem name"), v.at("number of variables")) {

	}
	F5::F5(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {
	...
	void F5::generateAdLoadPF() {
		...
		for (int i = 0; i < num; i++) {
			...
			o_point[i][1] = std::pow(1 - std::pow(o_point[i][0], 1/temp1),temp1);
			m_optima.append(o_point[i].vect());
		}
	}

	//f1=x1, f2=g*h
	EvalTag F5::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real t = get_time();
		...
		Real a = 2 * std::cos(OFEC_PI*t) + 2;
		Real b = 2 * std::sin(2 * OFEC_PI*t) + 2;
