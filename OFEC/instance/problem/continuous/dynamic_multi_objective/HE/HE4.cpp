...
namespace OFEC {
	HE4::HE4(param_map & v) : HE4(v.at("problem name"), v.at("number of variables")) {

	}
	HE4::HE4(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {
	...
	void HE4::generateAdLoadPF() {//the method to get Real PF is over sampling in 2 dimension
		int num = 1000;
		Real t = get_time();
	...
	EvalTag HE4::evaluateObjective(Real *x, std::vector<Real> &obj) {//recommend the number of variables is 20
		Real t = get_time();
		...
		for (size_t m = 0; m < m_num_objs; ++m)
		{
			if (m < 1) {
				...
				for (size_t i=2; i < m_num_vars; i += 2) {
					obj[m] += pow(x[i] - sin(6*OFEC_PI*x[0]+(i+1)*OFEC_PI/m_num_vars), 2);
				}
				...
				for (size_t i=1; i < m_num_vars; i += 2) {
					gt += pow(x[i] - sin(6 * OFEC_PI*x[0] + (i + 1)*OFEC_PI / m_num_vars), 2);
				}
