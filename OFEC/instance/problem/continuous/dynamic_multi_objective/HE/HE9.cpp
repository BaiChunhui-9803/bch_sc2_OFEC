...
namespace OFEC {
	HE9::HE9(param_map & v) : HE9(v.at("problem name"), v.at("number of variables")) {

	}
	HE9::HE9(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {
	...
	EvalTag HE9::evaluateObjective(Real *x, std::vector<Real> &obj) {//recommend the number of variables is 20
		Real t = get_time();
		...
		for (size_t m = 0; m < m_num_objs; ++m)
		{
			if (m < 1) {
				...
				for (size_t i=2; i < m_num_vars; i += 2) {
					...
					temp1 += y * y;
					temp2 *= std::cos(20*y*OFEC_PI/std::sqrt(i+1));
				}
				obj[m] = x[0] + 2. / (ceil(m_num_vars / 2.) - 1)*(4*temp1-2*temp2+2);
			}
			else {
				Real gt = 0;
				Real temp1 = 0, temp2 = 1;
				...
				gt = 2 - std::sqrt(x[0]) + 2. / floor(m_num_vars / 2.)*(4*temp1-2*temp2+2);
