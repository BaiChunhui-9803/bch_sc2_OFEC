...
namespace OFEC {
	class UDF8 final : public DMOPs {
	public:
		UDF8(const ParamMap &v);
		UDF8(const std::string &name, size_t size_var);
		~UDF8() {}
		void initialize();
		void generateAdLoadPF();
	private:
		void evaluateObjective(Real *x, std::vector<Real> &obj);
		std::vector<size_t> m_count = { 0,0,0,0,0 };
	};
}
