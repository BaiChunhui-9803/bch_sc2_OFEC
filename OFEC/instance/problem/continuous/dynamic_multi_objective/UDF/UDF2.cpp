...
namespace OFEC {
	UDF2::UDF2(param_map & v) : UDF2(v.at("problem name"), v.at("number of variables")) {

	}
	UDF2::UDF2(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {

	}

	void UDF2::initialize() {
	...
	EvalTag UDF2::evaluateObjective(Real *x, std::vector<Real> &obj) {//recommend the number of variables is 20
		Real t = get_time();
