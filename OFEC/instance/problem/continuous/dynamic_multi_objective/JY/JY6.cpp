...
namespace OFEC {
	JY6::JY6(param_map & v) : JY6(v.at("problem name"), v.at("number of variables")) {

	}
	JY6::JY6(const std::string & name, size_t size_var) : problem(name, size_var, 2), DMOPs(name, size_var, 2) {
	...
	EvalTag JY6::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real t = get_time();
		...
		Real At = 0.1;
		Real Wt = 3;
		Real Kt = 2 * std::floor(10*std::fabs(Gt));
		for (size_t i = 1; i < global::ms_global->m_problem->variable_size(); ++i){
			gt += 4*std::pow(x[i]-Gt,2)-std::cos(Kt*OFEC_PI*(x[i]-Gt))+1;
		}
