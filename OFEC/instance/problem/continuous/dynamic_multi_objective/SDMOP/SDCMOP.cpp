...
namespace OFEC {
	...
	SDCMOP::SDCMOP(const std::string &name, size_t size_var, size_t size_obj, size_t type, size_t size_peak) : problem(name, size_var, size_obj), DMOPs(name, size_var, size_obj) {

	}

	void SDCMOP::initialize() {
		for (size_t i = 0; i < m_num_vars; ++i) {//all variables are in [-1,1]
		...
		//num_pri = std::floor(m_num_vars/(Real)(m_num_objs-1));
		m_num_pri = 1;
		m_num_pub = m_num_vars - m_num_pri * (global::ms_arg.find("numObj")->second - 1);
		//m_num_pri = m_num_vars - m_num_pub;
		//m_num_PS = 2;
		m_num_PS = global::ms_arg.find("numPS")->second;

		m_constraint_type.resize(m_num_objs-1);
		for (auto &i : m_constraint_type) {
			i = constraint_type::Inequality;
		}

		//初始化H，W，m_x，R，需要将最高峰值放在第一个位置,H=[1,10],W=[5,45],R=[0.1,0.5]，m_x=[-1,1]
		for (size_t i = 0; i < m_num_objs - 1; i++) {
			...
			for (size_t j = 0; j < (size_t)global::ms_arg.find("numPeak")->second; j++) {
				...
				*(H_temp.begin()) = temp;//将最高峰放在第一个元素位置
				...
				for (size_t k = 0; k < m_num_vars; k++) {
					if (i*m_num_pri <= k && k < (i + 1)*m_num_pri || k >= (m_num_objs - 1)*m_num_pri)
		...
		for (size_t i = 0; i < m_num_PS; i++) {
			if (i < m_num_objs - 1) {
				...
				for (size_t j = 0; j < m_num_pub; j++) {
					//将子目标最优值的位置赋值给中心
					temp1.push_back(m_x0[i][0][m_num_pri*(m_num_objs - 1) + j]);
				}
			...
			//m_R.push_back(0.9 / (1 + std::exp(-1 * (m_num_objs - 3))));//PS半径与目标数耦合
	...
	void SDCMOP::generateAdLoadPF() {
		...
		***********************************************************************/
		...
		//Real t = get_time();
		//std::vector<std::vector<Real>> sample_PF = get_1d_sample_PF(m_num_objs);
		//matrix o_point(sample_PF[0].size(), m_num_objs);
		Real m_offset = std::min(1 - m_T / 2. * sign(2.* std::sqrt(m_num_objs - 1) / m_T - 2 * std::ceil(std::sqrt(m_num_objs - 1) / m_T) + 1)*std::pow(std::fabs(2 * std::sqrt(m_num_objs - 1) / m_T - 2. * std::ceil(std::sqrt(m_num_objs - 1) / m_T) + 1), m_alpha) - m_T * (std::ceil(std::sqrt(m_num_objs - 1) / m_T) - 1. / 2), 0.);
		size_t num;
		if (m_num_objs == 2) {
			//std::vector<std::vector<Real>> sam_PF = get_1d_sample_PF(2000);
			get_1d_sample_PF(2000);
			matrix o_point(m_sample_PF[0].size(), m_num_objs);
			...
			if (m_num_objs >= 8) {
				...
				num = std::ceil(-3.333*std::pow(m_num_objs, 3) + 58.93*std::pow(m_num_objs, 2) - 348.5*m_num_objs + 704.9);
			}
			...
			matrix o_point(sum, m_num_objs);
			//for (size_t j = 0; j < sum; j++) {
			//	Real temp = 0;
			//	for (size_t k = 0; k < m_num_objs - 1; k++) {
			//		size_t product = 1;
			//		for (size_t m = k + 1; m < m_num_objs - 1; m++) {
			...
			//	o_point[j][m_num_objs - 1] = 1 - m_T / 2. * sign(2.* temp / m_T - 2 * std::ceil(temp / m_T) + 1)*std::pow(std::fabs(2 * temp / m_T - 2. * std::ceil(temp / m_T) + 1), m_alpha) - m_T * (std::ceil(temp / m_T) - 1. / 2) - m_offset;;
	...
	void SDCMOP::get_1d_sample_PF(size_t n) {//n is the number of sample points in each objective dimension
		...
		for (size_t i = 0; i < m_num_objs - 1; i++) {
			...
			for (size_t q = 0; q < 2 * m_num_vars; q++) {
				...
				for (size_t q = 0; q < boundary_point.size(); q++) {
					...
					for (size_t j = 0; j < p; j++) {
						...
						for (size_t k = m_num_pri * i; k < m_num_pri*(i + 1); k++) {//private variables
							tmp2 += std::pow(boundary_point[q][k] - m_x[i][j][k], 2);//m_x is the optimum in this dimension
						}
						for (size_t m = m_num_pri * (m_num_objs - 1); m < m_num_vars; m++) {//public variables
							tmp2 += std::pow(boundary_point[q][m] - m_x[i][j][m], 2);
						}
		...
		std::vector<std::vector<std::pair<Real, Real>>> obj_range;
		std::vector<std::vector<std::pair<Real, Real>>> PS_obj_range;
		for (size_t i = 0; i < m_num_objs - 1; i++) {
			...
			for (size_t j = 0; j < m_num_PS; j++) {
				...
				for (size_t k = 0; k < p; k++) {
					...
					for (size_t m = 0; m < m_num_pub; m++) {
						temp += std::pow(m_x[i][k][m_num_pri*(m_num_objs - 1) + m] - m_center_of_PS[j][m], 2);
					}
			...
			obj_range.push_back(limit_value);

			//存储翻转后的范围，
			for (size_t j = 0; j < m_num_PS; j++) {
				Real temp;
				if (obj_range[i][j].first < m_obj_boundary_value[i])  //判断子目标取值下限是否小于边界翻折点的值
					if (obj_range[i][j].second < m_obj_boundary_value[i]) {
						temp = obj_range[i][j].first;
						obj_range[i][j].first = m_obj_boundary_value[i] - obj_range[i][j].second;
						obj_range[i][j].second = m_obj_boundary_value[i] - obj_range[i][j].first;
					}
					else if (m_obj_boundary_value[i] - obj_range[i][j].first > obj_range[i][j].second) {
						obj_range[i][j].second = m_obj_boundary_value[i] - obj_range[i][j].first;
						obj_range[i][j].first = 0;
					}
					else {
						obj_range[i][j].first = 0;
					}
			...
			for (size_t n = 0; n < m_num_PS - 1; n++) {
				...
				for (size_t p = 0; p < m_num_PS - 1 - n; p++) {
					if (obj_range[i][p].first>obj_range[i][p + 1].first) {
						temp = obj_range[i][p];
						obj_range[i][p] = obj_range[i][p + 1];
						obj_range[i][p + 1] = temp;
					}
				}
			}
		}
		PS_obj_range = obj_range;//每个PS在每个子目标下的取值范围

								 //再求排序之后的每个子目标的取值区间,将边界点保存
		std::vector<std::vector<Real>> m_boundary;
		for (size_t i = 0; i < m_num_objs - 1; i++) {
			Real min_min = obj_range[i][0].first;//存放所有PS区域中最小的下限值
			Real temp1 = obj_range[i][0].second;//存放下限值最小的PS所对应的上限值
			std::vector<Real> boundary_point;//保存片段的起点和终点值，元素个数为偶数
			boundary_point.push_back(std::fabs(min_min - m_obj_boundary_value[i]) / (m_H[i][0] - m_obj_boundary_value[i]));
			for (size_t j = 0; j < m_num_PS; j++) {
				if (obj_range[i][j].second > temp1) {
					if (obj_range[i][j].first > temp1) {
						boundary_point.push_back(std::fabs(temp1 - m_obj_boundary_value[i]) / (m_H[i][0] - m_obj_boundary_value[i]));//存入归一化的边界点
						boundary_point.push_back(std::fabs(obj_range[i][j].first - m_obj_boundary_value[i]) / (m_H[i][0] - m_obj_boundary_value[i]));
						min_min = obj_range[i][j].first;
						temp1 = obj_range[i][j].second;
					}
					else {
						temp1 = obj_range[i][j].second;
					}
				...
				if (j == m_num_PS - 1)
					boundary_point.push_back(std::fabs(temp1 - m_obj_boundary_value[i]) / (m_H[i][0] - m_obj_boundary_value[i]));
			}
			m_boundary.push_back(boundary_point);
		}
		...
		for (size_t i = 0; i < m_num_objs - 1; i++) {
			Real sum = 0;
			for (size_t j = 0; j < m_boundary[i].size() - 1;) {
				sum += m_boundary[i][j + 1] - m_boundary[i][j];
				j = j + 2;
			}
			m_sum.push_back(sum);
		}
		...
		m_average = m_average / (m_num_objs - 1);
		for (size_t i = 0; i < m_num_objs - 1; i++) {
		...
		for (size_t i = 0; i < m_num_objs - 1; i++) {
			...
			Real temp2;
			//第i个子目标采样
			for (int k = m_boundary[i].size() - 1; k > 0;) {
				temp2 = 1 - m_boundary[i][k];//最小化优化
				while (temp2 <= 1 - m_boundary[i][k - 1]) {
					m_point.push_back(temp2);
					temp2 += m_step;
				}
				m_point.push_back(1 - m_boundary[i][k - 1]);//加入每一段的端点
				k = k - 2;
			}
	...
	void SDCMOP::set_change_type(size_t i) {
		dynamic_factor temp = dynamic_factor(i);
		dynamic_type = temp;
	}

	void SDCMOP::update_problem(Real t) {
		...
		for (auto &i : s) {//依次索引测试项目的元素，
			...
			case dynamic_factor::Change_num_pub://只变公有变量的个数
			{
				...
				//num_pub = m_num_objs + std::floor((m_num_objs - 1) * std::sin(0.5*OFEC_PI*t));
				resize_variable(m_num_pri + m_num_pub);
				break;
			}
			case dynamic_factor::Change_location_peak://只变子目标峰的位置，同时要更新PS中心的位置
			{
				for (size_t i = 0; i < m_num_objs - 1; i++) {
					for (size_t j = 0; j < (size_t)global::ms_arg.find("numPeak")->second; j++) {
						for (size_t k = 0; k < m_num_vars; k++) {
							if (i*m_num_pri <= k && k < (i + 1)*m_num_pri || k >= (m_num_objs - 1)*m_num_pri)
								m_x[i][j][k] = m_x0[i][j][k] * std::cos(OFEC_PI*t);
						}
					}
					//更新PS中心的位置
					for (size_t k = 0; k < m_num_PS; k++) {
						for (size_t m = 0; m < m_num_pub; m++) {
							if (k < m_num_objs - 1) {
								//将子目标最优值的位置赋值给中心
								m_center_of_PS[k][m] = m_x[i][0][m_num_pri*(m_num_objs - 1) + m] + m_R[k] * std::sin(0.5*OFEC_PI*t);
							}
							else {
								//其余中心随机扰动
					...
					//R.push_back(0.5 / (1 + std::exp(-1 * (m_num_objs - 3))));//PS半径与目标数耦合
				}
				break;
			}
			...
			{
				size_t num_PS_t = 1 + std::ceil((m_num_objs - 1)*std::fabs(std::sin(0.1*OFEC_PI*t)));
				...
				if (temp>0) {
					...
					m_R.push_back(0.9 / (1 + std::exp(-1 * (m_num_objs - 3))));
				}
				else if (temp<0) {
			...
			case dynamic_factor::Change_radius_PS://只变PS的覆盖半径
			{
				for (size_t i = 0; i < m_R.size(); i++) {
					m_R[i] = 0.9 / (1 + std::exp(-1 * (m_num_objs - 3))) + 0.9 / (1 + std::exp(-1 * (m_num_objs - 3))) / 2 * std::sin(2 * OFEC_PI*t);
				}
				break;
			}
				...
				for (size_t i = 0; i < m_num_objs; i++) {
					for (size_t j = 0; j < (size_t)global::ms_arg.find("numPeak")->second; j++) {
						for (size_t k = 0; k < m_num_vars; k++) {
							if (i*m_num_pri <= k && k < (i + 1)*m_num_pri || k >= (m_num_objs - 1)*m_num_pri)
								if (i*m_num_pri <= k && k < (i + 1)*m_num_pri || k >= (m_num_objs - 1)*m_num_pri)
									m_x[i][j][k] = 0.5*m_x0[i][j][k] * std::cos(OFEC_PI*t) + 0.5*(2 * global::ms_global->m_uniform[caller::Problem]->next() - 1);
						}
					}
				}
				break;
	...
	void SDCMOP::evaluate_obj_nd_con(Real *x, std::vector<Real>& obj, std::vector<Real> &con) {
		Real t = get_time();
		...
		for (size_t i = 0; i < m_num_objs - 1; i++) {
			...
			for (size_t j = 0; j < p; j++) {
				Real temp2 = 0;
				for (size_t k = m_num_pri * i; k < m_num_pri*(i + 1); k++) {//private variables
					temp2 += std::pow(x[k] - m_x[i][j][k], 2);//m_x is the optimum in this dimension
					if (j == 0)//默认第一个位置为最高峰
						opt_pri.push_back(m_x[i][j][k]);
				}
				for (size_t m = m_num_pri * (m_num_objs - 1); m < m_num_vars; m++) {//public variables
					temp2 += std::pow(x[m] - m_x[i][j][m], 2);
					if (j == 0)//默认第一个位置为最高峰
						temp1.push_back(m_x[i][j][m]);
				}
		...
		for (size_t j = 0; j < m_num_PS; j++) {//public variables
			Real temp = 0;
			for (size_t k = 0; k < m_num_pub; k++) {
				temp += std::pow(x[m_num_pri*(m_num_objs - 1) + k] - m_center_of_PS[j][k], 2);
			}
		...
		obj[m_num_objs - 1] = g + (1 - m_F / std::pow(2, g) - m_offset);
		//obj[m_num_objs - 1] = g * (1 - m_T / 2. * sign(2.* F / m_T - 2 * std::ceil(F / m_T) + 1)*std::pow(std::fabs(2 * F / m_T - 2. * std::ceil(F / m_T) + 1), m_alpha) - m_T * (std::ceil(F / m_T) - 1. / 2) - m_offset);
