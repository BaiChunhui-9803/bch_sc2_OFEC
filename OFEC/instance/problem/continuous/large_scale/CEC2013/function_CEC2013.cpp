...
namespace OFEC {
	...
	void function_CEC2013::setOriginalGlobalOpt(Real *opt) {
		if (m_num_objs > 1) throw myexcept("function_CEC2013::set_original_global_opt only for problems with a single obj");
        Solution<VarVec<Real>, Real> s(m_num_objs, num_constraints(), m_num_vars);
		if (opt == 0)		for (auto&i : s.variable()) i = 0.;
		else	for (int i = 0; i < m_num_vars; i++)  s.variable()[i] = opt[i];
	...
	void function_CEC2013::setGlobalOpt(Real *tran) {
		if (m_num_objs > 1) throw myexcept("function_CEC2013::setGlobalOpt only for problems with a single obj");
        Solution<VarVec<Real>, Real> s(m_num_objs, num_constraints(), m_num_vars);
	...
	Real* function_CEC2013::readOvector()
	{
		...
		Real* d = new Real[m_num_vars];
		std::stringstream ss;
		ss << g_working_dir <<"/instance/problem/continuous/large_scale/CEC2013/data/" << "F" << ID << "-xopt.txt";
	...
	void function_CEC2013::create_random_permutation(std::vector<size_t> &vec) {
		...
		for (i = (m_num_vars << 3); i >= 0; --i) {
			j = global::ms_global->m_uniform[caller::Problem]->next_non_standard(0, (int)(m_num_vars-1));
			do {
				k = global::ms_global->m_uniform[caller::Problem]->next_non_standard(0, (int)(m_num_vars - 1));
			} while (k == j);
			t = vec[j];
			vec[j] = vec[k];
			vec[k] = t;
	...
	*/
	...
	{
		return (unsigned)(i* (2 * m_num_vars - i - 3) / 2 + j - 1);
	...
	{
		unsigned N = (unsigned)m_num_vars, indexCounter = 0;
