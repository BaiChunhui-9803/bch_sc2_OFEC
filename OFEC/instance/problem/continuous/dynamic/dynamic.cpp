...
//extern std::unique_ptr<Demo::scene> Demo::msp_buffer;
...
namespace OFEC {
	...
	Dynamic::Dynamic(const std::string &name ,size_t num_peaks,size_t num_objs = 1, size_t num_cons = 0) :Problem(name, num_objs, num_cons), m_counter(0)
	...
	void Dynamic::updateParameters() {
		...
		m_params["Flag of noise"] = m_flag_noise;
		m_params["Change frequency:"] = m_frequency;
	...
	void Dynamic::change() {
		...
		if (m_flag_num_peak_change) {
			if (m_mode == 1 || m_mode == 2) {
				...
				if (m_mode == 2) {
					step=GET_RND(m_id_rnd).uniform.nextNonStd(step / 2, 5 * step / 2);
				}
				...
				//random change
				m_temp_num_peak = GET_RND(m_id_rnd).uniform.nextNonStd(m_min_peaks, m_max_peaks);
			}
			changeNumPeak();
		}
...
//		msp_buffer->updateFitnessLandsacpe_();
	...
	Real Dynamic::getRecurrentNoise(int x, Real min, Real max, Real amplitude, Real angle, Real noisy_severity) {
		...
		noisy = noisy_severity * GET_RND(m_id_rnd).normal.next();
		t = y + noisy;
		if (t > min&&t < max) y = t;
		else y = t - noisy;
		return y;
