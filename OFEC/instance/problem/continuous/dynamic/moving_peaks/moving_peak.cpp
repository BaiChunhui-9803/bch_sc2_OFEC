...
namespace OFEC {
	...
	Real moving_peak::five_peak_basis_func(const Real *gen) {
		...
		for (int i = 0; i < 5; i++) {
			...
			for (int j = 1; j < m_num_vars; j++)  dummy += (gen[j] - basis_peak[i][j])*(gen[j] - basis_peak[i][j]);
			dummy = basis_peak[i][m_num_vars + 1] - (basis_peak[i][m_num_vars] * dummy);
			if (dummy > maximum)       maximum = dummy;
		}
		return maximum;
	}
	...
	Real moving_peak::peak_function_twin(const Real  *gen, int peak_number) /* two twin peaks moving together */
	{
		...
		dummy = m_height[peak_number] + twin_peak[m_num_vars + 1] - ((m_width[peak_number] + twin_peak[m_num_vars])*dummy);
		if (dummy > maximum)
			maximum = dummy;
