...
//#include "../../../../instance/algorithm/realworld/EPANET/AMP_CCSaDE/AMP_CCSaDE.h"
...
namespace OFEC {
	...
	class epanet : public problem, public mempool, public vars
	{
	protected:
		int m_phase = 1;
		...
		init_type m_init_type = init_type::k_means;
	...
	public:
		...
		std::vector<std::string> m_node_id;

		size_t m_pop_identifier;
		std::vector<Cluster> m_clusters;

		std::vector<int> m_num_be_visited;
		std::vector<double> m_pro_be_visited;
#ifdef OFEC_DEMO
		std::vector<std::vector<std::string>> m_connect;
#endif // OFEC_DEMO

	public:
		epanet(const std::string &name, size_t size_var, size_t size_obj, const std::string &file_name1, const std::string &file_name2, const std::string &file_name3);
		epanet(const ParamMap &v);
		~epanet() {
			//ENcloseQ();
			//ENclose(); 
		}
		...
		const long total_run_time() const {
			return m_total_duration;
		}
		const int num_sensor() const { return m_num_sensor; }
		...
		Real variable_distance(const solution_base &s1, const solution_base &s2) const { return 0.0; }
		Real variable_distance(const variable_base &s1, const variable_base &s2) const { return 0.0; }
		...
		const int max_start_time_size() {
			return (m_max_start_time - m_min_start_time) / m_pattern_step + 1;
		}
		const int max_duration_size() {
			return (m_max_duration - m_min_duration) / m_pattern_step + 1;
		}
		std::vector<Real> & get_coordinates(int idx);
		void cal_pro_by_be_visited();

		void set_init_type(init_type type) {
			m_init_type = type;
		}
	protected:
		void evaluateObjective(variable_epanet & x, std::vector<Real>& obj);
		void epanet_initialize();
	private:
