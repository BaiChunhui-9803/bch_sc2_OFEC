...
namespace OFEC {
	std::mutex mutex_epanet;
	//alloc_handle_t * vars::SegPool = nullptr;

	epanet::epanet(const std::string &name, size_t size_var, size_t size_obj, const std::string &file_name1, const std::string &file_name2, const std::string &file_name3): \
		problem(name, size_var, size_obj) {
		m_map_inp << g_working_dir << "/instance/problem/realworld/EPANET/data/map/" << file_name2;
		
		m_map_rpt << g_working_dir << "/instance/problem/realworld/EPANET/data/map/"<< global::ms_global.get()->m_runID + 1 << "_" << file_name3;

		m_file_name.assign(file_name1);

		m_init_type = init_type::random;

		/*m_clusters.resize(1);
		m_clusters[0].samples.resize(1);*/
	}
	epanet::epanet(const ParamMap &v) :epanet(v.at("problem name"), 1, 1, v.at("dataFile1"), v.at("dataFile2"), v.at("dataFile3")) {
	...
	void epanet::initialize() {
		...
		m_total_phase = m_total_duration / (m_time_interval*m_quality_time_step);
		m_phase = m_total_phase;
		
		get_data(m_optima.variable(0), m_Ciobs);
		
		std::stringstream path2;
		path2 << g_working_dir << "/result/new/Alg4Epa/AMP_CCSaDE/NET2_97/20w/best_obj.txt";
		std::ofstream fout2(path2.str(), std::ios::app);
		for (size_t i = 0; i < 144; ++i) {
			
				fout2 << m_Ciobs[0][i] << " " << m_Ciobs[1][i] << " " << m_Ciobs[2][i] << " " << m_Ciobs[3][i] << " " << (m_Ciobs[0][i] + m_Ciobs[1][i] + m_Ciobs[2][i] + m_Ciobs[3][i]) / 4.0 << std::endl;
			
		}
		fout2.close();

		m_num_be_visited.resize(m_num_node, 0);

		int first_detected_interval = m_optima.variable(0).first_detected_time() / m_quality_time_step;
		m_first_phase = first_detected_interval / m_time_interval + 1;
		...
		set_eval_monitor_flag(true);

		//m_phase = m_total_phase;
		//std::vector<Solution<variable_epanet, Real>> sol(1000,Solution<variable_epanet, Real>(1,1));
		//m_init_type = init_type::random;
		//for (size_t i = 0; i < sol.size(); ++i) {
		//	sol[i].initialize();
		//	sol[i].variable().duration() = 5400;
		//	//sol[i].variable().multiplier().resize(9);
		//	
		//	/*sol[i].variable().multiplier()[0] = 5;
		//	sol[i].variable().multiplier()[1] = 20;
		//	sol[i].variable().multiplier()[2] = 25;
		//	sol[i].variable().multiplier()[3] = 20;
		//	sol[i].variable().multiplier()[4] = 15;
		//	sol[i].variable().multiplier()[5] = 20;
		//	sol[i].variable().multiplier()[6] = 15;
		//	sol[i].variable().multiplier()[7] = 10;
		//	sol[i].variable().multiplier()[8] = 10;*/

		//	sol[i].evaluate();
		//}
		//std::stringstream path;
		//path << g_working_dir << "/result/test_dur_5400.txt";
		//std::ofstream fout(path.str(), std::ios::app);

		//for (size_t i = 0; i < sol.size(); ++i) {
		//	fout << sol[i].objective()[0] << std::endl;
		//}
	...
	void epanet::epanet_initialize() {
		...
		ENgetpatternindex(patternId, &m_patternIndex);
	...
	void epanet::read_parameter() {
		...
		std::ifstream infile;
		oss1 << g_working_dir << "/instance/problem/realworld/EPANET/data/input/"<< m_file_name;
	...
	EvalTag epanet::evaluate_(solution_base &s, caller call, bool effective, bool constructed) {
	...
	EvalTag epanet::evaluateObjective(variable_epanet & x, std::vector<Real>& obj) {    //  single evaluation
	...
	std::vector<Real> & epanet::get_coordinates(int idx) {
		return m_location[idx - 1];
	}
	...
	void epanet::get_data(variable_epanet &sol, std::vector<std::vector<float>> &data) {
		...
		ENsetpattern(m_patternIndex, patternValue.data(), m_num_pattern);

		for (int j = 1; j <= sol.multiplier().size(); ++j)
		{
			int period = sol.start_time() / m_pattern_step + j;
			float value = sol.multiplier()[j - 1];
			ENsetpatternvalue(m_patternIndex, period, value);
		}

		int len;
		ENgetpatternlen(m_patternIndex, &len);
		std::vector<float> pattern_data(len);  // for test
		for(size_t i=0;i<len;++i)
			ENgetpatternvalue(m_patternIndex, i, &pattern_data[i]);
		...
		ENsetnodevalue(sol.index(), EN_SOURCEQUAL, sol.source());//
		ENsetnodevalue(sol.index(), EN_SOURCEPAT, (float)m_patternIndex);//
		...
		do {
			...
			++count;
			if (count == m_time_interval*m_phase) break;  //
		} while (tstep > 0); //tstep=0��ʾģ�����
	...
	void epanet::initialize_solution(solution_base &s) const {
		...
		if (m_init_type == init_type::k_means) {
			int size_clusters = m_clusters.size();
			int random_identifier = m_pop_identifier;
			while (m_clusters[random_identifier].samples.size() <= 0) {
				random_identifier = global::ms_global->m_uniform[caller::Problem]->next_non_standard<int>(0, size_clusters);
			}
			int size = m_clusters[random_identifier].samples.size();
			int random = global::ms_global->m_uniform[caller::Problem]->next_non_standard<int>(0, size);
			var.index() = m_clusters[random_identifier].samples[random] + 1;
		}
		else if (m_init_type == init_type::random) {
			var.index() = global::ms_global->m_uniform[caller::Problem]->next_non_standard<int>(1, m_num_node+1);
		}
		else if (m_init_type == init_type::be_visited) {
			int size_clusters = m_clusters.size();
			int random_identifier = m_pop_identifier;
			while (m_clusters[random_identifier].samples.size() <= 0) {
				random_identifier = global::ms_global->m_uniform[caller::Problem]->next_non_standard<int>(0, size_clusters);
			...
			for (size_t i = 0; i < roulette.size() - 1; ++i) {
				roulette[i + 1] = roulette[i] + m_pro_be_visited[m_clusters[random_identifier].samples[i]];
			}
			Real random_value = global::ms_global->m_uniform[caller::Problem]->next_non_standard<Real>(roulette[0], roulette[roulette.size() - 1]);
			for (size_t i = 0; i < roulette.size() - 1; ++i) {
				if (random_value >= roulette[i] && random_value < roulette[i + 1]) {
					var.index() = m_clusters[random_identifier].samples[i] + 1;
				}
			}
		}
		else if (m_init_type == init_type::distance) {
		...
		var.duration() = const_cast<variable_epanet &>(m_optima.variable(0)).duration();
		//var.duration() = m_pattern_step * global::ms_global->m_uniform[caller::Problem]->next_non_standard<long>(m_min_duration / m_pattern_step, m_max_duration / m_pattern_step);
	...
	void epanet::cal_pro_by_be_visited() {
		int max = 0;
		for (auto &i : m_num_be_visited)
			if (max < i) max = i;
		...
		for (auto &i : inverse)
			m_pro_be_visited.push_back((double)i / (double)sum);
	}
	...
	*/
	...
	int  epanet::ENsetqualtype(int qualcode, char *chemname,
	...
	*/
	...
	int   epanet::openfiles(char *f1, char *f2, char *f3)
	/*----------------------------------------------------------------
	**  Input:   f1 = pointer to name of input file                  
	**           f2 = pointer to name of report file                 
	**           f3 = pointer to name of binary output file          
	...
	*/
	...
	int  epanet::strcomp(char *s1, char *s2)
	/*---------------------------------------------------------------
	...
	*/
	...
	STmplist * epanet::findID(char *id, STmplist *list)
		/*
		**-------------------------------------------------------------
		**  Input:   id = ID label
	...
	int  epanet::findmatch(char *line, char *keyword[])
		/*
		**--------------------------------------------------------------
	...
	int  epanet::match(char *str, char *substr)
		/*
		**--------------------------------------------------------------
	...
	double  epanet::hour(char *time, char *units)
		/*
		**---------------------------------------------------------
	...
	int  epanet::checkpremise(struct Premise *p)
		/*
		**----------------------------------------------------------
	...
	int  epanet::checktime(struct Premise *p)
		/*
		**------------------------------------------------------------
	...
	int  epanet::checkstatus(struct Premise *p)
		/*
		**------------------------------------------------------------
	...
	int  epanet::checkvalue(struct Premise *p)
		/*
		**----------------------------------------------------------
	...
	void  epanet::updateactlist(int i, struct Action *actions)
		/*
		**---------------------------------------------------
	...
	int  epanet::checkaction(int i, struct Action *a)
		/*
		**-----------------------------------------------------------
	...
	void epanet::writetime(char *fmt)
		/*
		**----------------------------------------------------------------
