...
#define DVRP_CAST(id_pro) dynamic_cast<DVRP::dynamic_vrp&>(GET_PRO(id_pro))
	...
	namespace DVRP {
		...
		class routes : public VarBase{//encode of solution
		public:
		...
		public:
			...
			void construct_path(SolBase &s);
			std::vector<bool> is_valid(SolBase &s) const;
		...
		private:
			...
			void construct_path(SolBase &s)const;
