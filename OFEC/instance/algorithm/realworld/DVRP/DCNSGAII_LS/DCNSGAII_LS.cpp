...
namespace OFEC {
	void DCNSGAII_LS_pop::initialize(int id_pro, int id_rnd) {
		...
		//m_ahc_tsp.randomly_init_pop(m_inds);
		//evaluate();
		///****************conference paper experiment********************************/
		//记录订单，对比订单是否相同
		std::string current_order_Fname;
		current_order_Fname = static_cast<std::string>(g_working_dir) + "/instance/algorithm/realworld/DVRP/Compare_data/current_order" + to_string(DVRP_CAST(id_pro).get_current_cus_order().size()) + "_" + m_algName_ + "_" + m_proName_ + "_" + m_initialName_ + ".txt";
		std::ofstream Corder_(current_order_Fname);
		std::stringstream Corder_str;
		for (auto &i : DVRP_CAST(id_pro).get_current_cus_order()) {
			Corder_str << i << " ";
		}
	...
	void DCNSGAII_LS_pop::sort() {
		...
		std::vector<IndType *> pop_;
		std::function<Dominance(IndType *const &, IndType *const &)> comp = std::bind(&DCNSGAII_LS_pop::Pareto_compare, this, _1, _2);
		for (auto &i : m_inds)
			pop_.emplace_back(i.get());
		NS::fast_sort<IndType *>(pop_, ranks_, comp);
		for (size_t i = 0; i < m_inds.size(); i++) {
			m_inds[i]->setRank(ranks_[i]);
		}
	}

	void DCNSGAII_LS_pop::Online(int id_pro, int id_alg, int id_rnd)
	{
		if (GET_PRO(id_pro).numObjectives() == 3)
			std::cout << "Offline WT: " << m_best_ind_Offline.objective()[2] << "\n";
		if (GET_PRO(id_pro).numObjectives() == 1)
			std::cout << "Offline WT: " << m_best_ind_Offline.constraint()[3] << "\n";
		DVRP_CAST(id_pro).set_Online(true);
		...
		for (int i = 0; i < m_best_ind_Online.variable().m_cus_order.size(); ++i) {
			double presentTime = DVRP_CAST(id_pro).get_depart_time() + DVRP_CAST(id_pro).get_cost_time(DVRP_CAST(id_pro).get_depart_time(), m_best_ind_Online.variable().m_cus_order[i][0], m_best_ind_Online.variable().m_cus_order[i][1]);
			for (int j = 1; j < m_best_ind_Online.variable().m_cus_order[i].size() - 1; ++j) {
				if (DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][j]]->datum_t.is_served == true)
					continue;
				DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][j]]->datum_t.is_served = true;
				...
				for (int k = 0; k < DVRP_CAST(id_pro).get_future_cus_order().size(); ++k) {
					if (presentTime >= DVRP_CAST(id_pro).get_future_cus_order()[k].second) {
						new_order = DVRP_CAST(id_pro).get_future_cus_order()[k];
						if (DVRP_CAST(id_pro).get_net().get_road_net()[new_order.first]->type != DVRP::node_type::future_customer || new_order.first == 175) {
							std::cout << "error" << "\n";
						}
						size_t next_cur_order = m_best_ind_Online.variable().m_cus_order[i][j + 1];
						//DVRP_CAST(id_pro).set_Online(false);
						presentTime = presentTime + DVRP_CAST(id_pro).get_net().get_road_net()[customer]->server_time + DVRP_CAST(id_pro).get_cost_time(presentTime, customer, new_order.first);
						if (presentTime >= DVRP_CAST(id_pro).get_net().get_road_net()[new_order.first]->datum_t.begin_time && presentTime <= DVRP_CAST(id_pro).get_net().get_road_net()[new_order.first]->datum_t.end_time) {

							m_best_ind_Online.variable().m_cus_order[i].insert(m_best_ind_Online.variable().m_cus_order[i].begin() + j + 1, new_order.first);
							DVRP_CAST(id_pro).get_future_cus_order().erase(DVRP_CAST(id_pro).get_future_cus_order().begin() + k);
							m_insert_success = true;
							if (m_best_ind_Online.variable().m_cus_order[i][0] != DVRP_CAST(id_pro).get_depot()) {
								std::cout << "error" << "\n";
							}
							updateSeqOnline_inserted(m_best_ind_Online, i, new_order, id_pro, id_alg, id_rnd);

							presentTime_newOrder = presentTime;
							presentTime = presentTime + DVRP_CAST(id_pro).get_net().get_road_net()[new_order.first]->server_time;
							presentTime = presentTime + DVRP_CAST(id_pro).get_cost_time(presentTime, new_order.first, next_cur_order);

							/*double delay_time = 0;
							if (presentTime > DVRP_CAST(id_pro).get_net().get_road_net()[next_cur_order]->datum_t.end_time) {
								delay_time = presentTime - DVRP_CAST(id_pro).get_net().get_road_net()[next_cur_order]->datum_t.end_time;
								...
								if (delay_time > m_best_ind_Offline.variable().m_hi.delay_time[i][nextOrder_pos - 1]) {
									...
									DVRP_CAST(id_pro).get_future_cus_order().push_back(new_order);
									m_insert_success = false;
								}
							}*/
							...
							if (m_insert_success == true) {
								...
								for (size_t m = 0; m < m_best_ind_Online.variable().m_cus_order[i].size(); ++m) {
									if (DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][m]]->datum_t.is_served == false)
										deliveryWeight += DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][m]]->datum_t.delivery_weight;
								}
								if (deliveryWeight > DVRP_CAST(id_pro).get_vehicle_property()[2].capacity) {
									over_load += deliveryWeight - DVRP_CAST(id_pro).get_vehicle_property()[2].capacity;
								}
								for (size_t m = 0; m < m_best_ind_Online.variable().m_cus_order[i].size(); ++m) {
									if (DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][m]]->datum_t.is_served == false)
										deliveryWeight = deliveryWeight - DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][m]]->datum_t.delivery_weight + DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][m]]->datum_t.pick_weight;
									else
										deliveryWeight = deliveryWeight - 0 + DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][m]]->datum_t.pick_weight;
						...
						//std::cout << m_best_ind_Online.objective()[0] << "\t" << m_best_ind_Online.constraint()[3] << "\t" << m_best_ind_Online.constraint()[2] << "\t" << m_best_ind_Online.constraint()[0] << "\t" << m_best_ind_Online.constraint()[1] << "\t" << std::to_string(DVRP_CAST(id_pro).get_future_cus_order().size()) << "\t" << m_best_ind_Online.variable().m_cus_order.size() << std::endl;
				...
				if (m_insert_success == false) {
					presentTime = presentTime_curOrder;
					presentTime = presentTime + DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][j]]->server_time + DVRP_CAST(id_pro).get_cost_time(presentTime, m_best_ind_Online.variable().m_cus_order[i][j], m_best_ind_Online.variable().m_cus_order[i][j + 1]);
				}
			}
			//标记已被服务
			for (int m = 1; m < m_best_ind_Online.variable().m_cus_order[i].size() - 1; ++m) {
				DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][m]]->datum_t.is_served = false;
			}
			for (int n = 1; n < m_best_ind_Online.variable().m_cus_order[i].size() - 1; ++n) {
				if (DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][n]]->type == DVRP::node_type::future_customer)
					break;
				DVRP_CAST(id_pro).get_net().get_road_net()[m_best_ind_Online.variable().m_cus_order[i][n]]->datum_t.is_served = true;
			}
			DVRP_CAST(id_pro).set_Online(true);
			construct_path_Online(m_best_ind_Online, i, id_pro);
		}
		m_best_ind_Online.evaluate(id_pro, id_alg);
		if (DVRP_CAST(id_pro).numObjectives() == 1)
			std::cout << "Online WT: " << m_best_ind_Online.constraint()[3] << "\n";
		if (DVRP_CAST(id_pro).numObjectives() == 3)
			std::cout << "Online WT: " << m_best_ind_Online.objective()[2] << "\n";
	}

	void DCNSGAII_LS_pop::construct_path_Online(IndType &ind, size_t i, int id_pro)
	{
		...
		Real present_time = DVRP_CAST(id_pro).get_depart_time();
		for (size_t j = 0; j < ind.variable().m_cus_order[i].size() - 1; j++) {
			std::vector<size_t> Shortest_path;

			DVRP_CAST(id_pro).shortest_path(ind.variable().m_cus_order[i][j], ind.variable().m_cus_order[i][j + 1], Shortest_path, present_time);//时间要变


			if (DVRP_CAST(id_pro).get_net().get_road_net()[ind.variable().m_cus_order[i][j]]->type == DVRP::node_type::current_customer || DVRP_CAST(id_pro).get_net().get_road_net()[ind.variable().m_cus_order[i][j]]->type == DVRP::node_type::future_customer) {
				present_time = present_time + DVRP_CAST(id_pro).get_cost_time(present_time, ind.variable().m_cus_order[i][j], ind.variable().m_cus_order[i][j + 1]) + DVRP_CAST(id_pro).get_net().get_road_net()[ind.variable().m_cus_order[i][j]]->server_time;
			}
			if (DVRP_CAST(id_pro).get_net().get_road_net()[ind.variable().m_cus_order[i][j]]->type == DVRP::node_type::depot) {
				present_time = present_time + DVRP_CAST(id_pro).get_cost_time(present_time, ind.variable().m_cus_order[i][j], ind.variable().m_cus_order[i][j + 1]);
			}
			auto tt = DVRP_CAST(id_pro).get_cost_time(present_time, ind.variable().m_cus_order[i][j], ind.variable().m_cus_order[i][j + 1]);
			...
			path_node_type_temp[j][0] = DVRP_CAST(id_pro).get_net().get_road_net()[path_temp[j][0]]->type;
			for (auto &m : path_node_type_temp[j]) {
				ind.variable().m_path_node_type[i].push_back(m);
			}
		}
		ind.variable().m_path[i].push_back(DVRP_CAST(id_pro).get_depot());
		ind.variable().m_path_node_type[i].push_back(DVRP_CAST(id_pro).get_net().get_road_net()[DVRP_CAST(id_pro).get_depot()]->type);
	}

	void DCNSGAII_LS_pop::updateSeqOnline(IndType &ind, size_t i, int id_pro, int id_alg, int id_rnd)
	{
		DVRP_CAST(id_pro).set_Online(false);

		DVRP_CAST(id_pro).construct_path(ind);
		...
		m_best_inds.clear();
		m_best_inds.emplace_back(new IndType(ind));
		calculate_initial_max_violation(m_best_inds);
		m_e = m_max_G;
		m_k = 0;
		std::cout << "updateSeqOnline() before: " << "\n";
		if (DVRP_CAST(id_pro).numObjectives() == 1)
			std::cout << ind.objective()[0] << "\t" << ind.constraint()[3] << "\t" << ind.constraint()[2] << "\t" << ind.constraint()[0] << "\t" << ind.constraint()[1] << "\t" << std::to_string(DVRP_CAST(id_pro).get_future_cus_order().size()) << "\t" << ind.variable().m_cus_order.size() << std::endl;
		if (DVRP_CAST(id_pro).numObjectives() == 3)
			std::cout << ind.objective()[0] << "\t" << ind.objective()[2] << "\t" << ind.objective()[1] << "\t" << ind.constraint()[0] << "\t" << ind.constraint()[1] << "\t" << std::to_string(DVRP_CAST(id_pro).get_future_cus_order().size()) << "\t" << ind.variable().m_cus_order.size() << std::endl;

		int iter_cnt = 0;
		while (iter_cnt < m_OnlineIters) {
			Online_ALS(ind, i, id_pro, id_alg, id_rnd);
			iter_cnt++;
		}
	...
	void DCNSGAII_LS_pop::updateSeqOnline_inserted(IndType &ind, size_t i, std::pair<int, int> new_order, int id_pro, int id_alg, int id_rnd)
	{
		DVRP_CAST(id_pro).set_Online(false);
		DVRP_CAST(id_pro).construct_path(ind);
		ind.evaluate(id_pro, id_alg);
		...
		while (iter_cnt < m_OnlineIters) {
			...
			if (DVRP_CAST(id_pro).numObjectives() == 3) {
				averge_delayTime = ind.objective()[1] / ind.variable().m_cus_order.size();//(DVRP_CAST(id_pro).get_current_cus_order().size() + (DVRP_CAST(id_pro).get_num_futureOrder() - DVRP_CAST(id_pro).get_future_cus_order().size()));;//m_best_ind_Online.constraint()[2] / (DVRP_CAST(id_pro).get_current_cus_order().size() + (DVRP_CAST(id_pro).get_num_futureOrder() - DVRP_CAST(id_pro).get_future_cus_order().size()));
			}
			if (DVRP_CAST(id_pro).numObjectives() == 1) {
				averge_delayTime = ind.constraint()[2] / ind.variable().m_cus_order.size();//(DVRP_CAST(id_pro).get_current_cus_order().size() + (DVRP_CAST(id_pro).get_num_futureOrder() - DVRP_CAST(id_pro).get_future_cus_order().size()));;//m_best_ind_Online.constraint()[2] / (DVRP_CAST(id_pro).get_current_cus_order().size() + (DVRP_CAST(id_pro).get_num_futureOrder() - DVRP_CAST(id_pro).get_future_cus_order().size()));
			}
			if (averge_delayTime > 45 && iter_cnt > 0.9 * m_OnlineIters) {
				if (DVRP_CAST(id_pro).get_num_futureOrder() - DVRP_CAST(id_pro).get_future_cus_order().size() == 0)
					break;
				...
				DVRP_CAST(id_pro).get_future_cus_order().push_back(new_order);
				auto fSize = DVRP_CAST(id_pro).get_future_cus_order().size();
	...
	void DCNSGAII_LS_pop::Online_ALS(IndType &ind, size_t i, int id_pro, int id_alg, int id_rnd)
	{
		...
		DCMOEA_ind<Individual<DVRP::routes>> best_ind_tmp = ind;
		...
		Real p = GET_RND(id_rnd).uniform.nextNonStd<Real>(0, 1);
		if (p <= m_cp_Online[0]) {
			ls_Rswap2_Online(ind, i, id_pro, id_rnd);
			operator_id = 0;
		}
		if (p > m_cp_Online[0] && p <= m_cp_Online[1]) {
			ls_maxDT_Online(ind, i, id_pro);
			operator_id = 1;
		}
		if (p > m_cp_Online[1] && p <= m_cp_Online[2]) {
			ls_2opt_Online(ind, i, id_pro, id_rnd);
			operator_id = 2;
		}
		DVRP_CAST(id_pro).construct_path(ind);
		ind.evaluate(id_pro, id_alg);
		...
		mark_individual_efeasible(m_best_inds);
		ind = *m_best_inds[0];
		...
		auto decrease_level = 50;//50 * exp(-pow(int(m_cnt_fail_Online), 0.5));//0.3*int(m_inds.size()) + 1 * 
	...
	void DCNSGAII_LS_pop::ls_Rswap2_Online(IndType &ind, size_t i, int id_pro, int id_rnd)
	{
		auto i_t = ind;
		int selected_car = i;
		int begin_pos = -1;
		for (int j = 1; j < ind.variable().m_cus_order[i].size(); ++j) {
			if (DVRP_CAST(id_pro).get_net().get_road_net()[ind.variable().m_cus_order[i][j]]->type == DVRP::node_type::depot)
				continue;
			if (DVRP_CAST(id_pro).get_net().get_road_net()[ind.variable().m_cus_order[i][j]]->datum_t.is_served == false)
			{
				begin_pos = j + 1;
				break;
			}
		...
		if (begin_pos == -1 || begin_pos == 0 || begin_pos == 1 || begin_pos == ind.variable().m_cus_order[selected_car].size() - 1 || begin_pos == ind.variable().m_cus_order[selected_car].size() - 2)
			return;
		//随机选1个顾客
		int selected_pos1 = GET_RND(id_rnd).uniform.nextNonStd<int>(begin_pos, ind.variable().m_cus_order[selected_car].size() - 1);

		int selected_pos2 = GET_RND(id_rnd).uniform.nextNonStd<int>(begin_pos, ind.variable().m_cus_order[selected_car].size() - 1);
		while (selected_pos1 == selected_pos2)
			selected_pos2 = GET_RND(id_rnd).uniform.nextNonStd<int>(begin_pos, ind.variable().m_cus_order[selected_car].size() - 1);
	...
	void DCNSGAII_LS_pop::ls_maxDT_Online(IndType &ind, size_t i, int id_pro)
	{
		auto i_t = ind;
		bool flag = true;
		...
		if (flag == true) {
			...
			if (ind.variable().m_cus_order[i][0] != DVRP_CAST(id_pro).get_depot()) {
	...
	void DCNSGAII_LS_pop::ls_2opt_Online(IndType &ind, size_t i, int id_pro, int id_rnd)
	{
		auto i_t = ind;
		if (ind.variable().m_cus_order[i][0] != DVRP_CAST(id_pro).get_depot()) {
		...
		ind.variable().m_cus_order[selected_car].push_back(DVRP_CAST(id_pro).get_depot());
	...
	DCNSGAII_LS_pop::DCNSGAII_LS_pop(size_t size_pop, int id_pro) :
		Population(size_pop, id_pro), 
		m_rand_seq(size_pop),
		m_best_ind_Online(GET_PRO(id_pro).numObjectives(), GET_PRO(id_pro).numConstraints()),
		m_best_ind_Offline(GET_PRO(id_pro).numObjectives(), GET_PRO(id_pro).numConstraints())
	{
		for (auto &i : m_inds) {
	...
	EvalTag DCNSGAII_LS_pop::evolve(int id_pro, int id_alg, int id_rnd) {
		...
		generate_offspring_ALS(id_pro, id_alg, id_rnd);
		...
		for (int i = 0; i < m_inds.size(); ++i) {
			//添加子代是否和父代个体完全相同的判断 
			Dominance is_dominated = Pareto_compare(m_offspring[i].get(), m_inds[i].get());
	...
	void DCNSGAII_LS_pop::select_next_parent_population(std::vector<IndType *> &pop, int id_pro) {
		/* Nondominated sorting based on e-Pareto domination  */
		std::vector<int> ranks;
		std::function<OFEC::Dominance(IndType *const &, IndType *const &)> comp = std::bind(&DCNSGAII_LS_pop::e_Pareto_compare, this, _1, _2);
		...
		if (id_ind < pop.size()) {
			...
			for (size_t j = 0; j < DVRP_CAST(id_pro).numObjectives(); j++) {
				for (size_t i = 0; i < s2; i++) {
					idd[i] = i;
	...
	Dominance DCNSGAII_LS_pop::e_Pareto_compare(IndType *const &s1, IndType *const &s2) {
	...
	void DCNSGAII_LS_pop::generate_offspring_LS(int id_pro, int id_alg, int id_rnd) {
		//local search
		for (int i = 0; i < m_inds.size(); ++i) {
			m_offspring[i]->solut() = *m_inds[i];
			m_offspring[i]->set_vio_obj(0, m_inds[i]->get_vio_obj()[0]);
			m_offspring[i]->set_efeasible(m_inds[i]->get_efeasible());
		}
		Dominance is_dominated;
		//size_t operator_id = 0;
		m_evo_success = false;
		...
		for (size_t i = 0; i < m_offspring.size(); ++i) {
			...
			Real p = GET_RND(id_rnd).uniform.nextNonStd<Real>(0, 1);
			if (p <= m_cp[0]) {
				ls_Rswap2(*m_offspring[i], id_pro, id_rnd);
				//operator_id = 0;
			}
			if (p > m_cp[0] && p <= m_cp[1]) {
				ls_tsp(*m_offspring[i], id_pro, id_rnd);
				//operator_id = 1;
			}
			if (p > m_cp[1] && p <= m_cp[2]) {
				ls_maxDT(*m_offspring[i], id_pro);
				//operator_id = 2;
			}
			if (p > m_cp[2] && p <= m_cp[3]) {
				ls_r1(*m_offspring[i], id_pro, id_rnd);
				//operator_id = 3;
			}
			if (p > m_cp[3] && p <= m_cp[4]) {
				ls_rMulti(*m_offspring[i], id_pro, id_rnd);
				//operator_id = 4;
			}
			if (p > m_cp[4] && p <= m_cp[5]) {
				ls_2opt(*m_offspring[i], id_pro, id_rnd);
				//operator_id = 5;
			}
			if (p > m_cp[5] && p <= m_cp[6]) {
				ls_maxWTlength(*m_offspring[i], id_pro);
				//operator_id = 6;
			}
			if (p > m_cp[6] && p <= m_cp[7]) {
				ls_maxLength(*m_offspring[i], id_pro, id_rnd);
				//operator_id = 7;
			}
			check_order(*m_offspring[i], id_pro);
			m_offspring[i]->evaluate(id_pro, id_alg);
			calculate_violation_objective(m_offspring);
			mark_individual_efeasible(m_offspring);

			is_dominated = Pareto_compare(m_offspring[i].get(), m_inds[i].get());
			auto a = double(i);
			if (m_convergence) {//double(i) > (int(m_inds.size()) / 1.5)
			...
			if (is_dominated == Dominance::Equal)
				m_cnt_fail++;
			//auto decrease_level = int(m_inds.size())*exp(-pow(int(m_cnt_fail), 0.5));//0.3*int(m_inds.size()) + 1 * 
			auto decrease_level = int(m_inds.size()) / 2.0;
	...
	void DCNSGAII_LS_pop::generate_offspring_ALS(int id_pro, int id_alg, int id_rnd) {
		std::string scoreFname;
		scoreFname = static_cast<std::string>(g_working_dir) + "/instance/algorithm/realworld/DVRP/Compare_data/score_" + to_string(m_inds.size()) + "_order" + to_string(DVRP_CAST(id_pro).get_current_cus_order().size()) + "ALSDCMOEA_VRPRTC_AHC_initial.txt";
		std::ofstream score(scoreFname, ios::app);
		std::stringstream score_;
		score_ << "iteration: " << to_string(m_iter) << "\n";
	...
	//	}
	//	DCMOEA_ind<Individual<DVRP::routes>> best_ind_tmp = m_best_ind_Online;
	...
	//	m_best_inds.clear();
	//	m_best_inds.emplace_back(new IndType(m_best_ind_Online));
	//	calculate_violation_objective(m_best_inds);
	//	mark_Individual_efeasible(m_best_inds);
	...
	//		m_cnt_fail_Online++;
	//	//auto b = exp(-pow(int(m_cnt_fail),0.5));
	//	auto decrease_level = 100;//50 * exp(-pow(int(m_cnt_fail_Online), 0.5));//0.3*int(m_inds.size()) + 1 * 
	//	if (double(m_cnt_fail_Online) >= decrease_level)
	//		m_convergence_Online = true;
	...
	void DCNSGAII_LS_pop::tsp_nearest(std::vector<size_t> &member_index, Real presentTime, int id_pro, int id_rnd)
	{
		std::vector<size_t> tsp;
		std::vector<std::tuple<size_t, Real, Real>> member_cost;
		size_t start = DVRP_CAST(id_pro).get_depot();
		...
		while (!member_index.empty()) {
			...
			for (int i = 0; i < member_index.size(); ++i) {
				...
				get_cost(start, member_index[i], present_time,id_pro,id_rnd);//此处耗时可能太长
				...
				Real diff_tw = 0.0;
				Real beginTime = DVRP_CAST(id_pro).get_net().get_road_net()[member_index[i]]->datum_t.begin_time;
				Real endTime = DVRP_CAST(id_pro).get_net().get_road_net()[member_index[i]]->datum_t.end_time;
			...
			Real co_d = GET_RND(id_rnd).uniform.nextNonStd<Real>(0, 1);
			for (int i = 0; i < member_index.size(); ++i) {
			...
			if (start == DVRP_CAST(id_pro).get_depot()) {
				Time = std::get<2>(member_cost[index]);
			}
			//Time = present_time;
			else {
				Time = std::get<2>(member_cost[index]) + DVRP_CAST(id_pro).get_net().get_road_net()[member_index[index]]->server_time;
			}
			//Time = present_time + DVRP_CAST(id_pro).get_net().get_road_net()[DVRP_CAST(id_pro).get_current_cus_order()[member_index[index]]]->server_time;
			start = next;
			cnt++;
	...
	void DCNSGAII_LS_pop::get_cost(size_t s, size_t e, Real &present_time, int id_pro, int id_rnd)
	{
		//present_time = present_time + DVRP_CAST(id_pro).get_cost_time(present_time, s, e);
		//present_time = present_time + DVRP_CAST(id_pro).get_cost_time()[s][e];
		present_time = present_time + DVRP_CAST(id_pro).get_cost_time(present_time, s, e);
	}
	void DCNSGAII_LS_pop::ls_Rswap2(IndType &offspring, int id_pro, int id_rnd)
	{
		int selected_car = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		while (offspring.variable().m_cus_order[selected_car].size() <= 3) {
			selected_car = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		}
		//随机选1个顾客
		int selected_pos1 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, offspring.variable().m_cus_order[selected_car].size() - 1);

		int selected_pos2 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, offspring.variable().m_cus_order[selected_car].size() - 1);
		while (selected_pos1 == selected_pos2)
			selected_pos2 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, offspring.variable().m_cus_order[selected_car].size() - 1);
		...
		if (selected_pos2 < offspring.variable().m_cus_order[selected_car].size() - 1) {
			...
			DVRP_CAST(id_pro).construct_path(offspring);
		}
		if (offspring.variable().m_cus_order[selected_car][0] != DVRP_CAST(id_pro).get_depot()) {
			std::cout << "ls_Rswap2() error" << "\n";
		}
	}

	void DCNSGAII_LS_pop::ls_tsp(IndType &offspring, int id_pro, int id_rnd)
	{
		int selected_car = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		while (offspring.variable().m_cus_order[selected_car].size() <= 3) {
			selected_car = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		...
		std::vector<size_t> seq_1, seq_2, seq_3;
		Real beginTime = 0;
		beginTime = DVRP_CAST(id_pro).get_depart_time();
		...
		offspring.variable().m_cus_order[selected_car].push_back(DVRP_CAST(id_pro).get_depot());
		...
		offspring.variable().m_cus_order[selected_car].push_back(DVRP_CAST(id_pro).get_depot());
		int selected_car2 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		while (selected_car2 == selected_car) {
			selected_car2 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		}
		...
		tsp_nearest(offspring.variable().m_cus_order[selected_car2], beginTime, id_pro, id_rnd);
		offspring.variable().m_cus_order[selected_car2].insert(offspring.variable().m_cus_order[selected_car2].begin(), DVRP_CAST(id_pro).get_depot());
		offspring.variable().m_cus_order[selected_car2].push_back(DVRP_CAST(id_pro).get_depot());
		DVRP_CAST(id_pro).construct_path(offspring);
		if (offspring.variable().m_cus_order[selected_car][0] != DVRP_CAST(id_pro).get_depot() || offspring.variable().m_cus_order[selected_car2][0] != DVRP_CAST(id_pro).get_depot()) {
			std::cout << "ls_tsp() error" << "\n";
		}

	}

	void DCNSGAII_LS_pop::ls_maxDT(IndType &offspring, int id_pro)
	{
		bool flag = true;
		...
		if (flag == true) {
			...
			if (offspring.variable().m_cus_order[max_delay_car][0] != DVRP_CAST(id_pro).get_depot() || offspring.variable().m_cus_order[max_wait_car][0] != DVRP_CAST(id_pro).get_depot()) {
				std::cout << "ls_maxDT() error" << "\n";
			}
		}
	}

	void DCNSGAII_LS_pop::ls_r1(IndType &offspring, int id_pro, int id_rnd)
	{
		int selected_car2 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		int selected_pos2 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, offspring.variable().m_cus_order[selected_car2].size() - 1);
		int selected_car = selected_car2;
		while (selected_car == selected_car2)
			selected_car = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		int selected_pos = GET_RND(id_rnd).uniform.nextNonStd<int>(1, offspring.variable().m_cus_order[selected_car].size() - 1);
	...
	void DCNSGAII_LS_pop::ls_rMulti(IndType &offspring, int id_pro, int id_rnd)
	{
		int cnt = GET_RND(id_rnd).uniform.nextNonStd<int>(1, 5);
		for (int i = 0; i < cnt; ++i) {
			int selected_car2 = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
			int selected_pos2 = GET_RND(id_rnd).uniform.nextNonStd<int>(1, offspring.variable().m_cus_order[selected_car2].size() - 1);
			int selected_car = selected_car2;
	...
	void DCNSGAII_LS_pop::ls_2opt(IndType &offspring, int id_pro, int id_rnd)
	{
		int selected_car = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
		while (offspring.variable().m_cus_order[selected_car].size() <= 3) {
			selected_car = GET_RND(id_rnd).uniform.nextNonStd<int>(0, offspring.variable().m_cus_order.size());
	...
	void DCNSGAII_LS_pop::ls_maxWTlength(IndType &offspring, int id_pro)
	{
		if (offspring.objective().size() == 1) {
		...
		if (max_wait_time != 0) {
			...
			DVRP_CAST(id_pro).construct_path(offspring);
			if (offspring.variable().m_cus_order[max_length_car][0] != 175 || offspring.variable().m_cus_order[max_wait_car][0] != DVRP_CAST(id_pro).get_depot()) {
	...
	bool DCNSGAII_LS_pop::check_order(IndType &offspring, int id_pro)
	{
		int num_order = 0;
		for (auto &i : offspring.variable().m_cus_order) {
			num_order += (int(i.size()) - 2);
		}
		if (num_order == DVRP_CAST(id_pro).get_current_cus_order().size())
			return true;
		else if (num_order < DVRP_CAST(id_pro).get_current_cus_order().size()) {
	...
	//	DVRP_CAST(id_pro).set_eval_monitor_flag(true);
	//}
	...
	void DCNSGAII_LS::initialize_() {
		...
		for (size_t i = 0; i < m_pop->size(); ++i) {
			pops[0].emplace_back(&m_pop->at(i).solut());
			//rank_.push_back(m_pop->at(i).rank());
		}
		dynamic_cast<Demo::buffer_DVRP *>(Demo::msp_buffer.get())->updateBuffer_(&pops, 0);
		dynamic_cast<Demo::buffer_DVRP *>(Demo::msp_buffer.get())->get_initial_pop(pops);
#endif
	}

	void DCNSGAII_LS::run_() {
		m_pop.reset(new DCNSGAII_LS_pop(m_pop_size, m_id_pro));
		m_pop->initialize(m_id_pro, m_id_rnd);
		m_pop->evaluate(m_id_pro, m_id_alg);
		m_pop->initializeAfterEvaluation();

		DCMOEA_ind<Individual<DVRP::routes>> best_ind = m_pop->at(0);
		DCMOEA_ind<Individual<DVRP::routes>> ini_best_ind = m_pop->at(0);
		m_pop->sort();
		...
		Real t_over_load = 0;
		Real t_over_time = 0;
		std::string initialPOP_Fname = static_cast<std::string>(g_working_dir) + "/instance/algorithm/realworld/DVRP/Compare_data/InitialPOP" + to_string(m_pop->size()) + "_order" + to_string(DVRP_CAST(m_id_pro).get_current_cus_order().size()) + "_" + m_pop->m_algName_ + "_" + m_pop->m_proName_ + "_" + m_pop->m_initialName_ + ".txt";
		std::ofstream pop_(initialPOP_Fname);
		std::stringstream pop_str;
		for (size_t i = 0; i < m_pop->size(); i++) {
			if (DVRP_CAST(m_id_pro).numObjectives() == 3) {
			...
			if (DVRP_CAST(m_id_pro).numObjectives() == 1) {
				t_route_length = m_pop->at(i).objective()[0];
			...
			pop_str << to_string(i) << "\t" << to_string(m_pop->at(i).rank()) << "\t" << t_route_length << "\t" << t_wait_time << "\t" << t_delay_time << "\t" << t_over_load << "\t" << t_over_time << "\t" << t_num_car << std::endl;
		}
		pop_ << pop_str.str();
		pop_.close();
		...
		while (!terminating()) {
			...
			if (iter >= 10000) {//10000  //&& domi_flag==Dominance::Dominant
				...
				m_pop->Online(m_id_pro, m_id_alg, m_id_rnd);
				std::string Online_resFname;
				Online_resFname = static_cast<std::string>(g_working_dir) + "/instance/algorithm/realworld/DVRP/Compare_data/Online_result" + to_string(m_pop->size()) + "_order" + to_string(DVRP_CAST(m_id_pro).get_current_cus_order().size()) + "_" + m_pop->m_algName_ + "_" + m_pop->m_proName_ + "_" + m_pop->m_initialName_ + ".txt";
				std::ofstream Online_result(Online_resFname);
				std::stringstream Online_result_;
				if (DVRP_CAST(m_id_pro).numObjectives() == 3) {
					Online_result_ << m_pop->get_bestInd().objective()[0] << "\t" << m_pop->get_bestInd().objective()[2] << "\t" << m_pop->get_bestInd().objective()[1] << "\t" << m_pop->get_bestInd().constraint()[0] << "\t" << m_pop->get_bestInd().constraint()[1] << "\t" << std::to_string(DVRP_CAST(m_id_pro).get_future_cus_order().size()) << "\t" << m_pop->get_bestInd().variable().m_cus_order.size() << std::endl;
					std::cout << m_pop->get_bestInd().objective()[0] << "\t" << m_pop->get_bestInd().objective()[2] << "\t" << m_pop->get_bestInd().objective()[1] << "\t" << m_pop->get_bestInd().constraint()[0] << "\t" << m_pop->get_bestInd().constraint()[1] << "\t" << std::to_string(DVRP_CAST(m_id_pro).get_future_cus_order().size()) << "\t" << m_pop->get_bestInd().variable().m_cus_order.size() << std::endl;
				}
				if (DVRP_CAST(m_id_pro).numObjectives() == 1) {
					Online_result_ << m_pop->get_bestInd().objective()[0] << "\t" << m_pop->get_bestInd().constraint()[3] << "\t" << m_pop->get_bestInd().constraint()[2] << "\t" << m_pop->get_bestInd().constraint()[0] << "\t" << m_pop->get_bestInd().constraint()[1] << "\t" << std::to_string(DVRP_CAST(m_id_pro).get_future_cus_order().size()) << "\t" << m_pop->get_bestInd().variable().m_cus_order.size() << std::endl;
					std::cout << m_pop->get_bestInd().objective()[0] << "\t" << m_pop->get_bestInd().constraint()[3] << "\t" << m_pop->get_bestInd().constraint()[2] << "\t" << m_pop->get_bestInd().constraint()[0] << "\t" << m_pop->get_bestInd().constraint()[1] << "\t" << std::to_string(DVRP_CAST(m_id_pro).get_future_cus_order().size()) << "\t" << m_pop->get_bestInd().variable().m_cus_order.size() << std::endl;
				}
			...
			for (size_t i = 0; i < m_pop->size(); ++i)
				pops[0].emplace_back(&m_pop->at(i).solut());
			dynamic_cast<Demo::buffer_DVRP*>(Demo::msp_buffer.get())->updateBuffer_(&pops);*/

			/* Begin terminal window output */
			size_t evals = DVRP_CAST(id_pro).evaluations();
			Real min_obj = (std::numeric_limits<Real>::max)();
			size_t idx_best;
			Real temp_obj;


			std::vector<std::pair<double, double>> objective_min_max(DVRP_CAST(id_pro).numObjectives());
			std::vector<std::pair<double, double>> constraint_min_max(DVRP_CAST(id_pro).num_constraints());
			std::vector<std::pair<double, double>> vioObj_min_max(m_pop->get_num_vio_obj());

			for (int i = 0; i < DVRP_CAST(id_pro).numObjectives(); ++i) {
			...
			for (int i = 0; i < DVRP_CAST(id_pro).num_constraints(); ++i) {
				std::pair<double, double> min_max;
				min_max.first = (std::numeric_limits<Real>::max)();
				min_max.second = (std::numeric_limits<Real>::min)();
				for (size_t j = 0; j < m_pop->size(); j++) {
			...
			result_Fname = static_cast<std::string>(g_working_dir) + "/instance/algorithm/realworld/DVRP/Compare_data/result_pop" + to_string(m_pop->size()) + "_order" + to_string(DVRP_CAST(id_pro).get_current_cus_order().size()) + "_" + m_pop->m_algName_ + "_" + m_pop->m_proName_ + "_" + m_pop->m_initialName_ + ".txt";
			...
			for (size_t i = 0; i < m_pop->size(); i++) {
				...
				//std::cout << "pop[" + to_string(i) << "]\t" << to_string(m_pop->at(i).rank()) << "\t" << route_length << "\t" << wait_time << "\t" << delay_time << "\t" << over_load << "\t" << over_time << "\t" << num_car << std::endl;
				result_ << to_string(i) << "\t" << to_string(m_pop->at(i).rank()) << "\t" << route_length << "\t" << wait_time << "\t" << delay_time << "\t" << over_load << "\t" << over_time << "\t" << num_car << std::endl;



				if (m_pop->at(i).rank() == 0) {
					if (delay_timeTmp >= delay_time) {
						delay_timeTmp = delay_time;
						idx_best = i;
					}
					/*temp_obj = 0;
					for (int j = 0; j < DVRP_CAST(id_pro).numObjectives(); ++j) {
					...
					for (int j = 0; j < DVRP_CAST(id_pro).num_constraints(); ++j) {
			...
			progress_Fname = static_cast<std::string>(g_working_dir) + "/instance/algorithm/realworld/DVRP/Compare_data/progress" + to_string(m_pop->size()) + "_order" + to_string(DVRP_CAST(id_pro).get_current_cus_order().size()) + "_" + m_pop->m_algName_ + "_" + m_pop->m_proName_ + "_" + m_pop->m_initialName_ + ".txt";
			...
			/*	std::cout << "Online" << std::endl;
				m_pop->Online();
				std::cout << m_pop->get_bestInd().objective()[0] << "\t" << m_pop->get_bestInd().constraint()[3] << "\t" << m_pop->get_bestInd().constraint()[2] << "\t" << m_pop->get_bestInd().constraint()[0] << "\t" << m_pop->get_bestInd().constraint()[1] << "\t" << std::to_string(DVRP_CAST(id_pro).get_future_cus_order().size()) << std::endl;
	*/
	...
	//system("pause");
			...
			for (size_t i = 0; i < m_pop->size(); ++i)
				pops[0].emplace_back(&m_pop->at(i).solut());
			dynamic_cast<Demo::buffer_DVRP *>(Demo::msp_buffer.get())->updateBuffer_(&pops, idx_best);
#endif
			iter++;
		}
		//m_pop->sort();
	...
	void DCNSGAII_LS::record() {
		...
		for (size_t i = 0; i < m_pop->size(); i++) {
			if (m_pop->at(i).rank() == 0) {
				temp_obj = 0;
				for (int j = 0; j < DVRP_CAST(m_id_pro).numObjectives(); ++j) {
					temp_obj += m_pop->at(i).objective()[j];
				}
				for (int j = 0; j < DVRP_CAST(m_id_pro).numConstraints(); ++j) {
					temp_obj += m_pop->at(i).constraint()[j];
				}
				for (int j = 0; j < m_pop->get_num_vio_obj(); ++j) {
					temp_obj += m_pop->at(i).get_vio_obj()[j];
