...
namespace OFEC {
	...
	public:
		...
		void clustering(Clustering_result &result, int id_pro);
	protected:
		LKH::LKHAlg lkh;
		...
		std::pair<Real, Real> tsp_nearest(std::vector<size_t> &member_index, int id_pro);
		std::pair<Real, Real> tsp_lkh(std::vector<size_t> &member_index, int id_pro);
		//void tsp_lkh(std::vector<size_t> &member_index, const std::vector<std::vector<Real>> &dis_matrix);
		void cal_tw_conflict(const std::vector<size_t> &member_index, std::vector<std::vector<Real>> &dis_matrix, int id_pro);
		void get_cost(size_t s, size_t e, Real &present_time, int id_pro);
		const bool is_valid(const std::vector<size_t> &cluster, const std::vector<size_t> &data, int id_pro) const;
	};
}
