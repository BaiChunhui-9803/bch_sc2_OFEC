...
namespace OFEC {
	...
	const std::vector<std::vector<Real>>& AHC_v2::cacul_dis_matrix(const std::vector<size_t> &data, int id_pro) {
		...
		for (int i = 0; i < data.size(); ++i) {
			for (int j = 0; j < data.size(); ++j) {
				...
				{
					d_conflict = cacul_d_conflict(data[i], data[j], id_pro);
					tw_conflict = cacul_tw_conflict(data[i], data[j], id_pro);
	...
	Real AHC_v2::cacul_d_conflict(size_t i, size_t j, int id_pro) {
		return DVRP_CAST(id_pro).get_shortest_dis(DVRP_CAST(id_pro).get_depart_time(),i,j);
	}

	Real AHC_v2::cacul_tw_conflict(size_t i, size_t j, int id_pro) {
		Real diff_tw = 0.0;
		size_t cus1 = i;
		size_t cus2 = j;

		Real cus1_beginTime = DVRP_CAST(id_pro).get_net().get_road_net()[cus1]->datum_t.begin_time;
		Real cus1_endTime = DVRP_CAST(id_pro).get_net().get_road_net()[cus1]->datum_t.end_time;
		Real cus2_beginTime = DVRP_CAST(id_pro).get_net().get_road_net()[cus2]->datum_t.begin_time;
		Real cus2_endTime = DVRP_CAST(id_pro).get_net().get_road_net()[cus2]->datum_t.end_time;
	...
	const bool AHC_v2::is_valid(const std::vector<size_t> &cluster, const std::vector<size_t> &data, int id_pro) const{
		bool is_over_load = 0;
		double deliveryWeight = 0;
