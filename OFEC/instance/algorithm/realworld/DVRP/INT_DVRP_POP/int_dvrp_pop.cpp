...
namespace OFEC {
	void INIT_DVRP_POP::ahc_tsp_init_pop(std::vector<DVRP_ind> &pop, int id_pro, int id_rnd) {
		...
		for (int i = 0; i < pop.size();++i ) {
			ahc_tsp_init_pop_(pop[i], i, id_pro, id_rnd);
		}
		...
		//	AHC_TSP ahc(weight, DVRP_CAST(id_pro).get_current_cus_order());
		...
		//		x.m_cus_order.back().push_back(DVRP_CAST(id_pro).get_depot());
		...
		//	}
		//	DVRP_CAST(id_pro).construct_path(*pop[i]);
			
		//}
		
	}
	void INIT_DVRP_POP::ahc_tsp_init_pop_(DVRP_ind& ind, int i, int id_pro, int id_rnd) {
		std::cout << "initialize pop " << std::to_string(i) << std::endl;
		auto &x = ind->variable();
		//随机生成权重
		Real weight = GET_RND(id_rnd).uniform.next();
		

		AHC_TSP::Clustering_result result;
		AHC_TSP ahc(weight, DVRP_CAST(id_pro).get_current_cus_order());
		ahc.clustering(result, id_pro);
	...
	void INIT_DVRP_POP::randomly_init_pop(std::vector<DVRP_ind>& pop, int id_pro, int id_rnd) {
		for (int i = 0; i < pop.size(); ++i) {
			std::cout << "initialize pop " << std::to_string(i) << std::endl;
			DVRP_CAST(id_pro).initSolution(*pop[i], id_rnd);
		}
	}

	void INIT_DVRP_POP::ahc_initialize_pop(std::vector<DVRP_ind>& pop, int id_pro, int id_rnd) {
		for (int i = 0; i < pop.size(); ++i) {
			auto &x = pop[i]->variable();
			Real weight = GET_RND(id_rnd).uniform.next();

			AHC_v2::Clustering_result result;
			AHC_v2 ahc(weight, DVRP_CAST(id_pro).get_current_cus_order());
			ahc.clustering(result, id_pro);

			std::vector<std::vector<size_t>> m_cus_order;
			m_cus_order = result.member;

			calcu_cus_order(m_cus_order, id_pro);
	...
	void INIT_DVRP_POP::calcu_cus_order(std::vector<std::vector<size_t>>& m_cus_order, int id_pro) {
		for (int i = 0; i < m_cus_order.size(); ++i) {
			m_cus_order[i] = tsp(m_cus_order[i], id_pro);
		}
	}
	
	std::vector<size_t> INIT_DVRP_POP::tsp(std::vector<size_t>& member_index, int id_pro) {
	...
	void INIT_DVRP_POP::get_cost(size_t s, size_t e, Real &present_time, int id_pro)
	{
