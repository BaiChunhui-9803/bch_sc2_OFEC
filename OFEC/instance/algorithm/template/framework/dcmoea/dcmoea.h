...
namespace OFEC {
	...
	public:
		...
		DCMOEA_ind(size_t num_obj, size_t num_con, Args&& ... args) :
			TIndividual(num_obj, num_con, std::forward<Args>(args)...) {}
	...
	protected:
		void calculate_initial_max_violation(std::vector<std::unique_ptr<TIndividual>> &pop);
		void mark_individual_efeasible(std::vector<std::unique_ptr<TIndividual>> &pop);
		bool judge_population_efeasible(std::vector<std::unique_ptr<TIndividual>> &pop);
		void reduce_boundary();
		virtual void calculate_violation_objective(std::vector<std::unique_ptr<TIndividual>> &pop);
	protected:
		Real m_z = 1.0e-08;
	...
	template<typename TIndividual>
	void DCMOEA<TIndividual>::calculate_initial_max_violation(std::vector<std::unique_ptr<TIndividual>> &pop) {
	...
	template<typename TIndividual>
	void DCMOEA<TIndividual>::mark_individual_efeasible(std::vector<std::unique_ptr<TIndividual>> &pop) {
		for (int i = 0; i < pop.size(); ++i) {
			bool test = true;
	...
	template<typename TIndividual>
	bool DCMOEA<TIndividual>::judge_population_efeasible(std::vector<std::unique_ptr<TIndividual>> &pop) {
		bool flag = true;
		for (int i = 0; i < pop.size(); ++i) {
	...
	template<typename TIndividual>
	void DCMOEA<TIndividual>::reduce_boundary() {
	...
	template<typename TIndividual>
	void DCMOEA<TIndividual>::calculate_violation_objective(std::vector<std::unique_ptr<TIndividual>> &pop) {
		for (int i = 0; i < pop.size(); ++i) {
			Real vObj = 0.0;
			for (int h = 0; h < m_num_cons; ++h) {
				vObj += (pop[i]->constraint()[h] / m_max_G[h]);
			}
			pop[i]->set_vio_obj(0, vObj / m_num_cons);
		}
	}
}
