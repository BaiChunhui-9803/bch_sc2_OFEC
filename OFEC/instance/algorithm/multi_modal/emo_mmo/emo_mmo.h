...
#include "../DE/Canonical_DE/Canonical_DE.h"
...
namespace OFEC {
	...
	public:
		...
		void update_norm_x(const std::vector<individual<>>& offspring);
		void update_delta(int t);
		void evaluate_2nd_obj(std::vector<individual<>>& offspring);
	private:
	...
	private:
		...
		SBX_pop<> m_MOFLA_pop;                          // population for MOFLA
		multi_population<DE::population<>> m_LS_pops;   // multipopulation for local search
