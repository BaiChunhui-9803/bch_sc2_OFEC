...
void plot_SPAE(const std::string& pro_name, std::vector<individual<>> &pop, HLC& hlc) {
	std::cout << "plot SPAE" << std::endl;
	for (auto& ind : pop)
		hlc.input_sample(ind.solut());
	hlc.clustering();
...
void plot_HSLH(const std::string& pro_name, std::vector<individual<>>& pop) {
	std::cout << "plot HSLH" << std::endl;
	HSLH<solution<>> hslh;
	std::vector<solution<>*> ptrs_pop;
	for (auto& ind : pop)
		ptrs_pop.push_back(&ind.solut());
...
void plot_APC(const std::string& pro_name, std::vector<individual<>>& pop) {
	std::cout << "plot APC" << std::endl;
	APC<individual<>> apc(0.9, 100, 30);
	apc.update_data(pop);
	apc.clustering();
...
void plot_NBC(const std::string& pro_name, std::vector<individual<>>& pop) {
...
void plot_APD(const std::string& pro_name, std::vector<individual<>>& pop) {
...
void initialize_pop(std::vector<individual<>>& pop, HLC& hlc) {
...
void evolve_pop(std::vector<individual<>>& pop, HLC& hlc) {
	std::cout << "evolve Population" << std::endl;
	DE::population<DE::individual> DE_pop(pop.size());
	for (size_t i = 0; i < pop.size(); ++i)
		DE_pop[i] = DE::individual(pop[i].solut());
	DE_pop.CR() = 0.9;
	DE_pop.F() = 0.5;
	...
	for (size_t iter = 0; iter < 50; iter++) {
		...
		for (size_t i = 0; i < DE_pop.size(); ++i) {
			if (DE_pop[i].is_improved())
				hlc.input_sample(DE_pop[i].solut());
			else
...
TEST_CASE("MMOP_CEC2013_F07", "[MMOP][CEC2013][F07][initial]")
{
	...
	global::ms_global->m_problem.reset(new CEC2013_MMOP_F07("MMOP_CEC2013_F07", 2));
...
TEST_CASE("MMOP_CEC2013_F09", "[MMOP][CEC2013][F09][initial]")
{
	...
	global::ms_arg["maximum evaluations"] = 200000;
	global::ms_global.reset(new global(1, 1./7, 0.5));
	global::ms_global->m_problem.reset(new CEC2013_MMOP_F09("MMOP_CEC2013_F09", 2));
	global::ms_global->m_problem->initialize();
...
TEST_CASE("GOP_CEC2005_F23", "[GOP][CEC2005][F23][initial]")
{
	...
	global::ms_global.reset(new global(1, 1./7, 0.5));
	global::ms_global->m_problem.reset(new CEC2005_GOP_F23("GOP_CEC2005_F23", 2, 1));
	global::ms_global->m_problem->initialize();
